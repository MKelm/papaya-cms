...
require_once(PAPAYA_INCLUDE_PATH.'system/Papaya/Ui/Reference/Page.php');
...
class PapayaUiReferencePageTest extends PapayaTestCase {
  ...
  */
  public function testGetWithParameters() {
    ...
    $this->assertEquals(
      'http://www.sample.tld/index.html?foo=bar',
      $reference->get()
    );
  }
  ...
  */
  public function testSetPageIdCallsConfigure() {
    ...
    $reference = new PapayaUiReferencePage();
    $reference->papaya(
      $this->getMockApplicationObject(array('pageReferences' => $pageReferences))
    );
    $reference->url($this->getUrlObjectMockFixture());
    $this->assertSame(
      $reference,
  ...
  */
  ...
  /**
  * @covers PapayaUiReferencePage::setFragment
  * @covers PapayaUiReferencePage::get
  */
