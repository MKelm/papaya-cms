...
class PapayaUiContentTeasersTest extends PapayaTestCase {
  ...
  public function testAppendToWithEmptyList() {
    ...
    $teasers = new PapayaUiContentTeasers($this->getPagesFixture());
    $teasers->papaya($this->getMockApplicationObject(array('plugins' => $plugins)));
  ...
  public function testAppendToWithPluginImplementingInterfaces() {
    ...
    $plugins
      ...
      ->with(
        array(
          '12345678901234567890123456789042'
        )
      );
    $plugins
      ->expects($this->once())
  ...
  public function testAppendToWithPluginAddingThumbnails() {
    ...
    $plugin = $this
      ->getMockBuilder('PapayaPluginQuoteable')
      ->setMockClassName('PapayaUiContentTeasers_PagePluginMockClassName')
      ->getMock();
    $plugin
      ->expects($this->once())
      ->method('appendQuoteTo')
      ->with($this->isInstanceOf('PapayaXmlElement'))
      ->will($this->returnCallback(array($this, 'callbackAppendTeaser')));
    ...
    $this->assertXmlStringEqualsXmlString(
      '<teasers>
        <teaser
          ...
          plugin="PapayaUiContentTeasers_PagePluginMockClassName"
...
abstract class PapayaUiContentTeasers_PagePluginMockClass
  implements PapayaPluginEditable, PapayaPluginQuoteable {
}