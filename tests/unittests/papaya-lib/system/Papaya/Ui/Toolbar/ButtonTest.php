...
require_once(PAPAYA_INCLUDE_PATH.'system/Papaya/Ui/Toolbar/Button.php');
...
class PapayaUiToolbarButtonTest extends PapayaTestCase {
  ...
  */
  public function testAppendTo() {
    ...
    $button = new PapayaUiToolbarButton();
    $button->papaya(
      $this->getMockApplicationObject(array('Images' => array('image' => 'sample.png')))
    );
    $button->caption = 'Test';
    $button->image = 'image';
  ...
  */
  public function testAppendToWithoutProperties() {
    ...
    $button = new PapayaUiToolbarButton();
    $button->papaya(
      $this->getMockApplicationObject(array('Images' => array('' => '')))
    );
    $button->appendTo($dom->documentElement);
    $this->assertEquals(
      '<sample/>',
