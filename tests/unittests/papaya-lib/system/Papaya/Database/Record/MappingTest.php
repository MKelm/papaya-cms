...
require_once(PAPAYA_INCLUDE_PATH.'system/Papaya/Database/Record/Mapping.php');
...
class PapayaDatabaseRecordMappingTest extends PapayaTestCase {
  ...
  */
  public function testMapPropertiesToFieldsGenericCallbacks() {
    ...
    $callbacks = $this
      ...
      ->setMethods(
        array('__isset', 'onBeforeMapping', 'onMapValue', 'onAfterMapping')
      )
      ->getMock();
    $callbacks
    ...
    $callbacks
      ->expects($this->once())
      ->method('onMapValue')
      ->with(PapayaDatabaseRecordMapping::PROPERTY_TO_FIELD, 'property_one', 'field_one', 42)
  ...
  */
  public function testMapPropertiesToFieldsSpecificCallbacks() {
    ...
    $callbacks
      ->expects($this->once())
      ->method('onMapValueFromPropertyToField')
      ->with('property_one', 'field_one', 42)
      ->will($this->returnValue(42));
    $callbacks
      ->expects($this->once())
  ...
  */
  public function testMapFieldsToPropertiesGenericCallbacks() {
    ...
    $callbacks
      ->expects($this->once())
      ->method('onMapValue')
      ->with(PapayaDatabaseRecordMapping::FIELD_TO_PROPERTY, 'property_one', 'field_one', 42)
      ->will($this->returnValue(42));
    $callbacks
      ->expects($this->once())
      ->method('onAfterMapping')
  ...
  */
  public function testMapFieldsToPropertiesSpecificCallbacks() {
    ...
    $callbacks
      ->expects($this->once())
      ->method('onMapValueFromFieldToProperty')
      ->with('property_one', 'field_one', 42)
      ->will($this->returnValue(42));
    $callbacks
      ->expects($this->once())
