...
require_once(PAPAYA_INCLUDE_PATH.'system/Papaya/Session.php');
...
class PapayaSessionTest extends PapayaTestCase {
  ...
  */
  public function testIsActiveAfterActivationExpectingTrue() {
    ...
    $_SERVER['HTTPS'] = NULL;
    $session = new PapayaSession();
    $session->papaya($this->getMockApplicationObject());
  ...
  */
  public function testIsSecureOnlyExpectingTrue($options) {
    ...
    $session->papaya(
      $this->getMockApplicationObject(
        array(
          'options' => $this->getMockConfigurationObject($options)
        )
      )
    );
  ...
  */
  public function testRegenerateId() {
    ...
    $session
      ...
      ->expects($this->once())
      ->method('regenerateId');
    $this->assertNull($session->regenerateId());
    $this->assertAttributeEquals(
