...
* @version $Id: base_auth.php 37834 2012-12-14 11:48:58Z smekal $
*/
...
/**
* Function class input verifying
*/
  ...
  function execLogin() {
    ...
    if (isset($this->params['chgpass']) &&
      ...
      if ($userId = $this->checkChangePasswordId($this->params['chgpass'])) {
        ...
        return $this->papaya()->messages->dispatch(
          new PapayaMessageDisplay(PapayaMessage::TYPE_ERROR, 'Invalid password change token')
        );
      }
  ...
  function login() {
    if (isset($this->params['login'])) {
      if ($this->checkLoginTry($this->params['username'])) {
        $passwordHash = $this->getPasswordHash($this->params['password']);
        if ($this->loadLogin($this->params['username'], $passwordHash)) {
          $this->logMsg(
            MSG_INFO,
            PAPAYA_LOGTYPE_USER,
            sprintf(
  ...
  */
  function loadLogin($username, $password) {
    ...
    if ($res = $this->databaseQueryFmt($sql, $params)) {
      if ($row = $res->fetchRow()) {
        $this->load($row[0], TRUE);
        $this->setSessionToken($row[0]);
      } elseif ($username == 'admin' &&
                ($password == '' || $password == $this->getPasswordHash(''))) {
        if ($this->addDefaultAdminUser()) {
          return $this->loadLogin($username, $this->getPasswordHash(''));
        }
      }
    }
    return $this->isValid;
  ...
  function load($uid, $login = FALSE) {
    ...
    if ($res = $this->databaseQueryFmt($sql, array($this->tableAuthUser, $uid))) {
      if ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($row['user_password'] == $this->getPasswordHash('')) {
          $this->addMsg(
            MSG_WARNING,
    ...
    if ($chkUserId = $this->userExists($this->params['username'])) {
      if ($chkUserId == $userId) {
        if (trim($this->params['password']) != '') {
          $data = array(
            'user_password' => $this->getPasswordHash($this->params['password']),
            'chg_id' => '',
            'chg_time' => '0'
          );
    ...
    if (isset($checkFunctions) && is_array($checkFunctions) &&
      ...
      foreach ($fields as $key => $val) {
        ...
        if (in_array($checkFunction, $checkFunctions)) {
          ...
          if ($checkObj->$checkFunction($paramValue, $val[2])) {
  ...
  */
  function getLoginXML() {
    if (isset($this->layout) && is_object($this->layout)) {
      ...
      $this->layout->setParam('PAGE_TITLE', $this->_gt('Login'));
      $this->layout->setParam('PAGE_ICON', $this->images['status-system-locked']);
      $this->layout->setParam('PAGE_USER', $this->_gt('Unknown'));
    ...
    if (isset($this->params['send_password']) &&
      ...
      if ($this->loadByUserName($this->params['username'])) {
        ...
        if ($this->saveChangePasswordId($this->userId, $chgId)) {
          ...
          if (checkit::isEMail($this->user['email'], TRUE)) {
            $email->send();
          }
        }
      }
      ...
      $result .= '</login>';
      $this->layout->setParam('PAGE_TITLE', $this->_gt('Password forgotten'));
      $this->layout->setParam('PAGE_ICON', $this->images['status-system-locked']);
      $this->layout->setParam('PAGE_USER', $this->_gt('Unknown'));
      $this->layout->add($result);
    } else {
  ...
  function changeForgottenPassword($userId) {
    ...
    if (isset($this->user)) {
      ...
      if ($changed) {
        ...
        $this->layout->setParam('PAGE_TITLE', $this->_gt('Change password'));
        $this->layout->setParam('PAGE_ICON', $this->images['status-system-locked']);
        $this->layout->setParam('PAGE_USER', $this->_gt('Unknown'));
        $this->layout->add($result);
      }
