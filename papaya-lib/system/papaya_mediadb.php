...
require_once(PAPAYA_INCLUDE_PATH.'system/papaya_paging_buttons.php');
require_once(PAPAYA_INCLUDE_PATH.'system/base_tags.php');
...
class papaya_mediadb extends base_mediadb_edit {
  ...
  */
  function execute() {
    if (isset($_GET['upload_progress_id'])) {
      include_once(PAPAYA_INCLUDE_PATH.'system/papaya_mediadb_rpc.php');
  ...
  */
  function executeFilesManagement() {
    ...
    $this->lngId = $this->lngSelect->currentLanguageId;
    $this->loadFileData();
    $this->loadFolderData();

    if (isset($this->msgs) && isset($this->params['msgs'])) {
      $this->msgs->restoreFromSession($this->params['msgs']);
    }
    ...
    if (!empty($this->params['cmd'])) {
      ...
      case 'upload':
        ...
        if (isset($this->params['confirm']) && $this->params['confirm'] &&
          ...
          if (isset($_POST['UPLOAD_JAVASCRIPT']) && $_POST['UPLOAD_JAVASCRIPT']) {
            if (isset($this->msgs)) {
              $msgIdent = $this->msgs->saveToSession();
            }
            printf(
              '<html><body><script type="text/javascript">'.
              'parent.uploadFinished("?%s[cmd]=edit_file&%s[msgs]=%s");</script></body></html>',
              papaya_strings::escapeHTMLChars($this->paramName),
              papaya_strings::escapeHTMLChars($this->paramName),
              papaya_strings::escapeHTMLChars($msgIdent)
            );
            exit;
          } else {
      ...
      case 'edit_file':
        if (isset($this->currentFile['file_id']) &&
          ...
          if (isset($this->params['confirm']) && $this->params['confirm'] &&
              $this->dialog->checkDialogInput()) {
            if ($this->updateFile($this->params, $this->lngSelect->currentLanguageId)) {
          ...
          if (isset($this->params['file_id'])) {
            if (isset($this->params['confirm']) && isset($this->params['version_id'])) {
              if ($this->restoreVersion($this->params['file_id'], $this->params['version_id'])) {
                ...
                $this->addMsg(MSG_ERROR, $this->getErrorForType($this->lastError));
              }
            } else {
      ...
      case 'batch_delete':
        if (isset($this->params['batch_files']) && count($this->params['batch_files']) > 0) {
          if (isset($this->params['confirm']) && $this->params['confirm']) {
            if ($this->deleteMultipleFiles(array_keys($this->params['batch_files']))) {
  ...
  * @param mixed $folderId a folder id or an array thereof (either this or fileId must have a value)
  *
  */
  ...
  function getFolderPanel() {
    ...
    if (!(
      ...
      $result .= sprintf(
        ...
        papaya_strings::escapeHTMLChars($this->images[$folderIcon]),
        papaya_strings::escapeHTMLChars(
  ...
  */
  function getClipboardPanel() {
    $result = '';
    $folders = $this->getSubFolders($this->lngSelect->currentLanguageId, -1, TRUE);
    $files = $this->getFiles(
  ...
  */
  function getFilesPagingBar($absCount, $step, $offset, $offsetSuffix = 'files') {
    ...
    $result .= papaya_paging_buttons::getButtons(
      ...
      array(
        'list' => array(
          $this->_gt('List'), $this->images['categories-view-list']
        ),
        'tile' => array(
          $this->_gt('Tiles'), $this->images['categories-view-tiles']
        ),
        'thumbs' => array(
          $this->_gt('Thumbnails'), $this->images['categories-view-icons']
        )
      ),
    ...
    if (isset($this->params['file_id']) && $this->params['file_id'] != '') {
      ...
      if (is_array($file) && count($file) > 0) {
        if ($file['folder_id'] > 0) {
          ...
          if (isset($folder[$this->lngSelect->currentLanguageId])) {
            $folderName = $folder[$this->lngSelect->currentLanguageId]['folder_name'];
          } elseif (isset($folder[PAPAYA_CONTENT_LANGUAGE])) {
            $folderName = $folder[PAPAYA_CONTENT_LANGUAGE]['folder_name'];
          }
        ...
        if (is_array($mimeType) &&
          ...
          $info = sprintf(
            ...
            papaya_strings::escapeHTMLChars($this->images['status-dialog-warning'])
          );
        } else {
          $info = '';
        }
  ...
  * @param array $derivations list of derivations
      ...
      foreach ($groups as $groupId => $groupTitle) {
        if (isset($folderPerms['user_edit'][$groupId])) {
          if ($folderPerms['user_edit'][$groupId] == 'own') {
            $addFileIcon = $this->images['status-node-checked'];
            ...
            $addFileIcon = $this->images['status-node-checked-disabled'];
            $addFileLink = '';
          }
        } else {
          if ($folder['permission_mode'] != 'inherited') {
            $addFileIcon = $this->images['status-node-empty'];
            $addFileLink = $this->getLink(
              array(
                'cmd' => 'edit_folder',
            ...
            $addFileIcon = $this->images['status-node-empty-disabled'];
            $addFileLink = '';
          }
        }
        if (isset($folderPerms['user_view'][$groupId])) {
          if ($folderPerms['user_view'][$groupId] == 'own') {
            $viewFileIcon = $this->images['status-node-checked'];
            ...
            $viewFileIcon = $this->images['status-node-checked-disabled'];
            $viewFileLink = '';
          }
        } else {
          if ($folder['permission_mode'] != 'inherited') {
            $viewFileIcon = $this->images['status-node-empty'];
            $viewFileLink = $this->getLink(
              array(
                'cmd' => 'edit_folder',
                'perm_action' => 'add_perm',
            ...
            $viewFileIcon = $this->images['status-node-empty-disabled'];
            $viewFileLink = '';
          }
        }
        $result .= sprintf(
      ...
      if (is_array($surferPerms) && count($surferPerms) > 0) {
        ...
        foreach ($surferPerms as $permId => $permission) {
          ...
          if ($permission['surferperm_active'] == 1) {
            $activeIcon = $this->images['status-node-checked-disabled'];
          } else {
            $activeIcon = $this->images['status-node-empty-disabled'];
          }
          $result .= sprintf(
            '<subitem align="center"><glyph src="%s" /></subitem>'.LF,
  ...
  function addNewFolder() {
    ...
    if ($folderId) {
      ...
      $saved = $this->addFolderTranslation(
        $folderId, $this->lngSelect->currentLanguageId, $this->params['folder_name']
      );
      if ($saved) {
  ...
  *
  * @TODO use base_auth::getSurferId() instead
  *
  ...
  */
  function initializeGetFileFromWebDialog() {
    ...
    $this->dialog = new base_dialog($this, $this->paramName, $fields, $data, $hidden);
    $this->dialog->tokenKeySuffix = 'filefromweb';
    $this->dialog->dialogTitle = $title;
    $this->dialog->buttonTitle = 'Save';
    $this->dialog->inputFieldSize = 'large';
  }
    ...
    if (is_file($srcFileName)) {
      if ($this->initializeImageConverter($srcFileName)) {
        ...
        if ($this->imageConverter->canConvert($srcFormat)) {
          ...
          $this->dialog->tokenKeySuffix = 'convert_image';
          $this->dialog->loadParams();
        } else {
          $this->addMsg(
            MSG_ERROR,
  ...
  * @return string|FALSE
  */
  function convertImage($fileId, $targetFormat, $folderId = -1) {
    if (!$this->loadSurferData()) {
      return FALSE;
    }
    ...
    if ($this->imageConverter->canConvert($srcFormat, $targetFormat)) {
      ...
      if ($this->imageConverter->convert($srcFileName, $tempFileName, $targetFormat)) {
        ...
        if ($newFileId = $this->addFile($tempFileName, $fileName, -1, $this->surfer['surfer_id'])) {
...
HEREDOC;
  ...
  function moveMultipleFiles($fileIds, $targetFolderId) {
    if ($this->checkActionPermission('edit_file', NULL, $targetFolderId)) {
      ...
      if ($targetFolderId == -1) {
        ...
        if (isset($folderData[$this->lngSelect->currentLanguageId])) {
          $folderName = $folderData[$this->lngSelect->currentLanguageId]['folder_name'];
  ...
  *
  * @param string $path location of folder to check
