...
* @version $Id: base_messages.php 33865 2010-03-22 17:17:33Z siddi $
*/
...
class base_messages extends base_db {
  ...
  */
  function loadMessages() {
    ...
    $params = array($this->tableMessages,$this->authUser->userId,
  ...
  */
  function loadMessage($id) {
    ...
    $params = array($this->tableMessages, (int)$id, $this->authUser->userId);
    if ($res = $this->databaseQueryFmt($sql, $params)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        $this->message = $row;
      }
  ...
  function decodeAddresses($str) {
    ...
    if (preg_match_all($addressPattern, $str, $regs, PREG_SET_ORDER)) {
      foreach ($regs as $reg) {
        ...
        } elseif (checkit::isEMail($userEmail, TRUE)) {
          $result[] = array(
            'name' => $userName,
            'email' => $userEmail
          );
  ...
  * @access public
  */
  function saveMessage($addrList, $data) {
