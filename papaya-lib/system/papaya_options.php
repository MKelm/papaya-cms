...
* @package Papaya
* @subpackage Core
* @version $Id: papaya_options.php 37654 2012-11-08 16:20:49Z weinert $
*/
...
class papaya_options extends base_options {
  ...
  */
  ...
  function initializeSearchDialog() {
    if (!(isset($this->searchDialog) && is_object($this->searchDialog))) {
      ...
      $this->searchDialog->tokenKeySuffix = 'searchdialog';
  ...
  */
  function import() {
    ...
    if ($doc->nodeType != XML_ELEMENT_NODE || $doc->nodeName != 'options') {
  ...
  */
  function checkDirectoryWriteable($path, $name) {
    ...
    if (is_dir($path)) {
      umask(011);
      if (@chmod($path, 0777)) {
        $this->addMsg(MSG_INFO, $this->_gtf('%s is writeable.', $title));
        return TRUE;
      } elseif (is_writeable($path)) {
        $this->addMsg(MSG_INFO, $this->_gtf('%s is writeable.', $title));
        return TRUE;
      } else {
        $this->addMsg(MSG_INFO, $this->_gtf('Cannot modify %s.', $title));
      }
    } else {
