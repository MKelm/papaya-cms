...
* @subpackage Strings
* @version $Id: papaya_strings.php 36907 2012-03-29 15:51:01Z weinert $
*/
...
class papaya_strings {
  ...
  * @return string
  */
  function entityToXML($str, $escapeTags = FALSE) {
    $result = $str;
    $result = preg_replace(
  ...
  * @return string
  */
  function escapeHTMLTags($str, $nl2br = FALSE) {
    $replace = array('<', '>');
    $replaceWith = array('&lt;', '&gt;');
    $offset = 0;
    $status = 0;
  ...
  * @param string $str
  * @param integer $start
  * @param mixed integer $length optional, default value NULL
  * @access public
  * @return string
  */
  ...
  */
  function normalizeString($utf8String, $maxLength = 0, $language = NULL) {
    return PapayaUtilFile::normalizeName($utf8String, $maxLength, $language);
  }

  /**
  * transliterate an utf-8 string to ascii - ignoring invalid chars
  *
  * this function is an embedded version of the utf8_to_ascii project
  * (@link http://phputf8.sourceforge.net/) and uses the original data files
  *
  * unlike the original function
  * it ignores invalid utf8 chars and does not use an output filter
  *
  * @param string $str
  * @param string $unknown unknown chars are replaced with this char (not used any more)
  * @param string $srcLanguage transliterate from this language
  * @access public
  * @return string
  */
  function transliterate($str, $unknown = '-', $srcLanguage = NULL) {
    $converter = new PapayaStringTransliterationAscii();
    return $converter->transliterate($str, $srcLanguage);
  }
  ...
  * @return string
  */
  function utf8ToCharset($utf8String, $charset) {
  ...
  */
  function truncate($str, $length = 80, $sep = ' ', $continue = '...') {
  ...
  * @return string $result truncated string or input string if shorter than $length
  */
  function truncateFileName($str, $length = 80, $continue = '..') {
  ...
  * @return string
  */
  function unicodeEntitiesToUTF8($str) {
  ...
  * @return string
  */
  function unicodeEntitiesToUTF8_PHP4Callback($match) {
  ...
  * @since 2008-07-09
  */
  function isoDateTimeToUTS($date, $timezone = 'local') {
  ...
  * @since 2008-07-09
  */
  function utsToIsoDateTime($uts, $timezone = 'local') {
    switch (strtolower($timezone)) {
    case 'gmt':
  ...
  * @return string
  */
  function base32_decode($encodedString) {
  ...
  * @return array
  */
  function fgetcsv($handle, $length, $delimiter = ',', $enclosure = '"', $escape = NULL) {
    if (is_null($escape)) {
      $escape = $enclosure;
    }
