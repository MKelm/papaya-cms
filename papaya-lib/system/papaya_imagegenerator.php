...
* @version $Id: papaya_imagegenerator.php 36477 2011-12-03 13:25:26Z weinert $
*/

/**
* image generator basic class
*/
require_once(PAPAYA_INCLUDE_PATH.'system/base_imagegenerator.php');
/**
* papaya object to configure dynamic imageConfs
*
* @package Papaya
  ...
  function executeImageConfModuleEdit() {
    if (isset($this->imageConf) && isset($this->imageConf['module_guid'])) {
      ...
      $moduleObj = &base_pluginloader::getPluginInstance(
        ...
        $parent,
        NULL,
        $this->imageConf['module_class'],
        $this->imageConf['module_path'].$this->imageConf['module_file']
      );
      if (isset($moduleObj) && is_object($moduleObj)) {
        $moduleObj->images = &$this->images;
        $moduleObj->paramName = $this->paramName;
        $moduleObj->setData($this->imageConf['image_data']);
        $hidden = array(
  ...
  */
  function initializeImageConfDialog() {
    if (!(isset($this->dialogImageConf) && is_object($this->dialogImageConf))) {
      ...
      $systemImageFormat = $this->papaya()->options->getOption('PAPAYA_THUMBS_FILETYPE');
      $imageFormats = array(
      ...
      $systemCacheTime = $this->papaya()->options->getOption('PAPAYA_CACHE_TIME_FILES');
      $cacheModes = array(
        0 => $this->_gt('No Cache'),
      ...
      $this->dialogImageConf->msgs = &$this->msgs;
