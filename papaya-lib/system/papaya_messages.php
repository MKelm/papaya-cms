...
* @version $Id: papaya_messages.php 37908 2012-12-21 11:08:04Z elbrecht $
*/
...
*/
require_once(PAPAYA_INCLUDE_PATH.'system/base_messages.php');
...
class papaya_messages extends base_messages {
  ...
  */
  function getFolderPanel() {
    ...
    $result .= sprintf(
      ...
      papaya_strings::escapeHTMLChars($this->images['items-task']),
    ...
    $result .= sprintf(
      ...
      papaya_strings::escapeHTMLChars(
        ($counts[0] > 0)
          ? $this->images['status-messages-inbox-full']
          :  $this->images['categories-messages-inbox']
      ),
    ...
    $result .= sprintf(
      ...
      papaya_strings::escapeHTMLChars(
        ($counts[-1] > 0)
          ? $this->images['status-messages-outbox-full']
          : $this->images['categories-messages-outbox']
      ),
    ...
    $result .= sprintf(
      '<icon src="%s" subtitle="%s" href="%s"/>',
      papaya_strings::escapeHTMLChars(
        ($counts[-2] > 0) ? $this->images['status-trash-full'] :  $this->images['places-trash']
      ),
      ...
      foreach ($this->messageList as $message) {
        ...
        if ($priorityIndex = $this->getPriorityGlyphIndex($message['msg_priority'])) {
          $result .= sprintf(
            ...
            papaya_strings::escapeHTMLChars($this->images[$priorityIndex])
          );
        } else {
          $result .= '<subitem/>';
        }
  ...
  */
  function getFolderGlyph($index) {
    switch ($index) {
    case 0:
      return $this->images['categories-messages-inbox'];
    case -1:
      return $this->images['categories-messages-outbox'];
    case -2:
      return $this->images['places-trash'];
    case -3:
      return $this->images['items-taks'];
    default:
      return '';
    }
  }
  ...
  function addrStrToJs($str, $field) {
    ...
    if (isset($addrs) && is_array($addrs)) {
      ...
      foreach ($addrs as $key=>$val) {
        $address = (isset($val['user_id'])) ?
          $val['user_id'] : $val['email'];
        $result .= sprintf(
          'addr["%s"][%s] = "%s <%s>";'.LF,
          $field,
          $key,
...
var msg_wasadded = "'.$alertMessage.'";
  ...
  */
  function sendMessage() {
    ...
    if ($this->checkAndCompleteAddresses($addr)) {
      ...
      if (checkit::isEmail($this->authUser->user['email'], TRUE)) {
        $email->setSender(
          $this->getUserEmailAddress($this->authUser->user),
          $this->getUserEmailName($this->authUser->user));
      }
      ...
      $record = array(
        ...
        'msg_from' => empty($this->authUser->userId) ? '' : (string)$this->authUser->userId,
        'msg_datetime'=> time(),
        ...
        'msg_priority'=> empty($this->params['priority']) ? '0' : (string)$this->params['priority'],
      ...
      //Postausgang
      $record['msg_owner_id'] = $this->authUser->userId;
        ...
        foreach ($list as $key => $val) {
          if (isset($val['email'])) {
            ...
            if ($field == 'to') {
              ...
              checkit::isEmail($this->users[$val['user_id']]['email'], TRUE)) {
            $val['email'] = $this->users[$val['user_id']]['email'];
            $resultAddresses[$field][] = $val;
            if ($field == 'to') {
              $result = TRUE;
            }
          } elseif (isset($val['user_login']) &&
                    isset($userNames[$val['user_login']]) &&
                    checkit::isEmail($this->users[$userNames[$val['user_login']]]['email'], TRUE)) {
  ...
  */
  function getButtons() {
    ...
    if (isset($this->message) && is_array($this->message)) {
      $userFrom = $this->users[$this->message['msg_from']];
      $toolbar->addButton(
        'Answer',
