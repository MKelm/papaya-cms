...
* @package Papaya
* @subpackage Core
* @version $Id: sys_base_object.php 38496 2013-05-17 10:32:04Z weinert $
*/
...
* message type info
*/
define('MSG_INFO', PapayaMessage::TYPE_INFO);

/**
* message type warning
*/
define('MSG_WARNING', PapayaMessage::TYPE_WARNING);

/**
* message type error
*/
define('MSG_ERROR', PapayaMessage::TYPE_ERROR);
...
* @subpackage Core
*/
class base_object extends PapayaObject {
  /**
  * Error message
  ...
  * @access public
  */
  function logVariable($level, $group, $title, $variable,
                       $addBacktrace = FALSE, $backtraceOffset = 3) {
  ...
  * @access public
  */
  function addMsg($level, $text, $log = FALSE, $group = PAPAYA_LOGTYPE_SYSTEM) {
    $this->papaya()->messages->dispatch(
  ...
  * @return array
  */
  function parseRequestURI($url = NULL) {
    $application = &$this->papaya();
    if (empty($url)) {
      $request = &$application->getObject('Request');
    } else {
  ...
  */
  function getAbsoluteURL($url, $text = '', $sid = TRUE, $protocol = NULL) {
    ...
    if (!($host = $urlObject->getHost())) {
      $host = $this->papaya()->options->get('PAPAYA_DEFAULT_HOST', 'localhost');
    }
    $baseHref = $protocol."://".$host;
  ...
  function getXHTMLString($str, $nlToBr = FALSE) {
    ...
    if (trim($iStr) == '') {
      ...
      if (!$fragment->appendXml($iStr)) {
        $showErrors = PapayaApplication::getInstance()->options->getOption(
