...
* @package Papaya
* @subpackage Core
* @version $Id: papaya_output.php 38135 2013-02-18 15:15:38Z weinert $
*/
  ...
  function sendHeader() {
    if (isset($this->viewMode)) {
      ...
      if (isset($GLOBALS['PAPAYA_PAGE']) && is_object($GLOBALS['PAPAYA_PAGE'])) {
        $GLOBALS['PAPAYA_PAGE']->sendHeader($headerStr);
      } else {
        header($headerStr);
      }
    }
  ...
  function &getFilter($viewId) {
    ...
    if (isset($this->viewMode) &&
      ...
      $result = &base_pluginloader::getPluginInstance(
        ...
        $parent,
        NULL,
        $this->viewLink['module_class'],
        $this->viewLink['module_path'].$this->viewLink['module_file']);
      if ($result) {
        if (!empty($this->viewMode['viewmode_path'])) {
          $result->templatePath = $this->viewMode['viewmode_path'];
        }
        $result->setData($this->viewLink['viewlink_data']);
      }
    }
    return $result;
  }
