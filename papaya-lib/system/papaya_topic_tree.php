...
* @version $Id: papaya_topic_tree.php 38024 2013-01-25 17:12:47Z weinert $
*/


/**
* Basic classe base_topictree
*/
require_once(PAPAYA_INCLUDE_PATH.'system/base_topictree.php');
/**
* Classe papaya_topic.inc
*/
require_once(PAPAYA_INCLUDE_PATH.'system/papaya_topic.php');
...
class papaya_topic_tree extends base_topic_tree {
    ...
    if (isset($this->params['cmd'])) {
      ...
      case 'publish':
        if ($pageId > 0) {
          ...
          $topic->lngSelect = &$this->lngSelect;
          $topic->params = $this->params;
          if ($topic->load($pageId, $this->lngSelect->currentLanguageId)) {
      ...
      case 'mv':
        $success = FALSE;
        $this->load($this->authUser->startNode, $this->lngSelect->currentLanguageId);
        ...
        if (isset($topic) && is_array($topic)) {
          if ($this->topicEditable($pageId) &&
              ($this->topicEditable($targetId) || $targetId == 0)  &&
              $this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_MOVE)) {
      ...
      case 'cp':
        ...
        if (isset($topic) && is_array($topic)) {
          if (($this->topicEditable($targetId) || $targetId == 0) &&
              $this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_COPY)) {
            if (!$this->authUser->hasPerm(
    ...
    if ($target['topic_id'] == $topic['topic_id'] ||
      ...
      if ($res = $this->databaseQueryFmt($sql, $params)) {
        while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
          ...
          $row['author_id'] = $this->authUser->userId;
      ...
      if (isset($this->copyTopics) && is_array($this->copyTopics)) {
        ...
        if ($newId = $this->databaseInsertRecord(
          ...
          if ($success = $this->copyPageTranslations(
                $topic['topic_id'], $newId, $newPath)) {
  ...
  * @return boolean
  */
  function copyPageTranslations($oldId, $newId, $newPath) {
          ...
          if ($newId = $this->databaseInsertRecord(
            ...
            if ($success = $this->copyPageTranslations($id, $newId, $newPath)) {
              ...
              if (isset($this->copyTopicsTree[$id]) &&
                ...
                $success = $this->copySubPages(
                  $this->copyTopicsTree[$id], $newId, $newPath);
              }
            }
          } else {
            $success = FALSE;
  ...
  */
  function initPartTree(&$topic) {
    ...
    $this->load($base, $lngSelect->currentLanguageId);
    $nodes = array();
  ...
  */
  function get() {
    ...
    if (isset($this->topics) && is_array($this->topics)) {
      ...
      if ($this->topicId > 0) {
        ...
        $result .= sprintf(
          ...
          papaya_strings::escapeHTMLChars($this->images['actions-edit-copy']),
      ...
      $counter = 0;
      if (isset($this->topicLinks[$this->authUser->startNode])) {
        $result .= $this->getSubTree(
          $this->topicLinks[$this->authUser->startNode]['children'],
          $counter
        );
      }
      $result .= '</items></listview>'."\r\n";
    ...
    $topic = new papaya_topic;
    if ($topic->load($this->params['page_id'], $this->lngSelect->currentLanguageId)) {
        ...
        if (isset($val) && is_array($val)) {
          ...
          if ($showPublish &&
            ...
            $result .= sprintf(
              ...
              papaya_strings::escapeHTMLChars($this->images['items-publication']),
