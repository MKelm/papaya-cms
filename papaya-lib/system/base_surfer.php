...
* @version $Id: base_surfer.php 38470 2013-04-30 14:56:54Z kersken $
*/
...
class base_surfer extends base_db {
  ...
  */
  ...
  /**
  * Surfer data
  * @var mixed array or null $surfer
  */
  var $surfer = NULL;
  ...
  * @param boolean $requestPassword optional, default value FALSE
  */
  public function executeLogin($requestPassword = FALSE) {
    $this->baseLink = $this->getBaseLink();
    ...
    if (isset($_POST[$this->logformVar.'_logout']) &&
      ...
      if (!$this->isValid) {
        ...
        if (isset($_POST[$this->logformVar.'_email']) ||
          ...
          if (isset($handle) &&
              trim($handle) != '' &&
              $handle != $this->getSessionValue($this->surfernameVar)) {
            $this->setSessionValue($this->loginVar, 'id');
          ...
          if (isset($login) && trim($login) != '') {
            if (checkit::isEMail($login)) {
              if ($login != $this->getSessionValue($this->surfermailVar)) {
                $this->setSessionValue($this->loginVar, 'id');
                // successful login will terminate script after login()
                $this->login($login, $password, $redirection);
              }
            } else {
              if ($login != $this->getSessionValue($this->surfernameVar)) {
                $this->setSessionValue($this->loginVar, 'id');
                // successful login will terminate script after loginByHandle()
  ...
  /**
  * Starting to introduce some new dynamic properties, that will used in the refactored object.
  *
  * @param string $name;
  * @return mixed
  */
  ...
  * @return boolean
  */
  function loadLoginBy($field, $value, $login = FALSE, $password = '', $requestPassword = FALSE) {
    ...
    if (trim($field) != '') {
      ...
      if ($res = $this->databaseQueryFmt($sql, $sqlParams)) {
        if ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
          ...
          $loginValid = ($field == 'surfer_relogin') ||
            ($this->getPasswordHash($password) == $row['surfer_password']);
          if (!$login || ($loginPossible && $loginValid)) {
            ...
            $this->isValid = ($row['surfer_valid'] == 1);
          ...
          } elseif ($this->getPasswordHash($password) != $row['surfer_password']) {
            if ($requestPassword === FALSE) {
              $this->errors[SURFER_ERROR_PASSWORD] = 'Invalid password';
            }
          } elseif (!(bool)$row['surfer_valid']) {
            $this->errors[SURFER_ERROR_BLOCKED] = 'Account blocked';
          } else {
            $this->errors[SURFER_ERROR_UNKNOWN] = 'Unknown Error';
          }
          // }
        } else {
          if ($requestPassword === FALSE) {
  ...
  * @return boolean
  */
  function load($surferId, $surferHandle) {
  ...
  */
  function redirectOnLogin($redirectionUrl) {
    ...
    $session = $this->papaya()->session;
    if ($redirect = $this->papaya()->session->regenerateId($targetUrl)) {
      $redirect->send();
      exit();
    }
  }
  ...
  */
  ...
  /**
  * Login by handle (legacy option for old papaya installations)
  *
  * @param string $surferHandle
  * @param string $password
  * @param string $redirectionURL optional url to redirect to after login
  ...
  * @return boolean login valid?
  */
  function loginByHandle($surferHandle, $password,
                         $redirectionURL = NULL, $requestPassword = FALSE) {
  ...
  */
  function loadSurferGroupsList($forceReload = FALSE) {
    if (!isset($this->surferGroups) || ($forceReload && isset($this->surferGroups))) {
      $this->surferGroups = array();
    }
    if (empty($this->surferGroups)) {
      ...
      if ($this->saveChangePasswordId($userId, $chgId)) {
        ...
        if ((!empty($userEmail) && $aSurfer->loadLogin($userEmail, FALSE, '', TRUE)) ||
          ...
          if (isset($mail_subject) && trim($mail_subject) != '') {
            ...
            $subject = papaya_strings::escapeHTMLChars(PAPAYA_PROJECT_TITLE).' - '.
              'Password forgotten';
          }
          if (isset($mail_body) && trim($mail_body) != '') {
            $msg = $mail_body."\n\n";
          } else {
            $msg = "Sie erhalten diese E-Mail, da eine Passwortaenderung fuer ".
              PAPAYA_PROJECT_TITLE." angefordert wurde. ";
            $msg .= "Falls Sie Ihr Passwort nicht aendern moechten,".
              " koennen Sie diese E-Mail ignorieren.\n\n";
            $msg .= "Um Ihr Passwort zu aendern, klicken Sie auf den folgenden Link:\n\n";
          }
          ...
          $email->setBody($mail_body, $fillValues, 70);
          $success = $email->send();
          if ($success) {
            return TRUE;
          }
  ...
  * @param string $queryString optional Query string for the redirection URL
  * @access public
  */
  ...
  * @return string
  */
  function getFormXML($login = TRUE, $redirectionURL = NULL,
      $emailTitle = 'E-Mail', $passwordTitle = 'Password',
      $reloginTitle = 'Stay logged-in', $errors = array()) {
    if ($login) {
  ...
  * @return string
  */
  function getHandleFormXML($login = TRUE, $redirectionURL = NULL,
      $handleTitle = 'Username', $passwordTitle = 'Password',
      $reloginTitle = 'Stay logged-in', $errors = array()) {
    if ($login) {
      // If there was a delegated error from the login box, add it here
      if ((
  ...
  * @return string
  */
  function getEmailOrHandleFormXML($login = TRUE, $redirectionURL = NULL,
      $handleTitle = 'Username or email', $passwordTitle = 'Password',
      $reloginTitle = 'Stay logged-in', $errors = array()) {
    if ($login) {
      // If there was a delegated error from the login box, add it here
      if ((
