...
* @package Papaya
* @subpackage Core
* @version $Id: base_csv.php 33298 2009-12-08 15:30:27Z zerebecki $
*/
...
class base_csv extends base_object {
  ...
  */
  function readCSVFile($fileLocation, $firstRowTitles = TRUE, $associative = TRUE,
                       $rows = NULL, $offset = 0) {
    $result = FALSE;
    if ($this->checkCSVFile($fileLocation)) {
      ...
      if ($fp = fopen($fileLocation, 'r')) {
        ...
        while ($row = papaya_strings::fgetcsv(
                $fp, 8192, $csvStyle['seperator'], $csvStyle['delimiter']
               )) {
  ...
  */
  function readCSVData(&$csvData, $firstRowTitles = TRUE, $associative = TRUE) {
    $cacheId = md5($csvData);
    $tempFileLocation = PAPAYA_PATH_CACHE.'.csvd_'.$cacheId.'.csv';
  ...
  *
  * @todo improve check and return false if no valid style could be determined
  *
  * @access private
  ...
  */
  function outputCSV(&$csvData, $fileName) {
