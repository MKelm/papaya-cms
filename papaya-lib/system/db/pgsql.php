...
* @subpackage Database
* @version $Id: pgsql.php 38333 2013-03-27 15:04:01Z weinert $
*/
...
class dbcon_pgsql extends dbcon_base {
  ...
  */
  public function lastInsertId($table, $idField) {
    $sql = "SELECT CURRVAL('".$tableName."_".$idField."_seq')";
  ...
  */
  function insertRecords($table, $values) {
    $baseSQL = "COPY ".$this->escapeStr($table)." ";
    $lastFields = NULL;
      ...
      foreach ($values as $data) {
        if (is_array($data) && count($data) > 0) {
          ...
          foreach ($data as $key => $val) {
            $fields[] = strtr($this->escapeStr($key), $specialChars);
            if ($val === '') {
              $valueData[] = '';
            } else {
              $valueData[] = strtr($this->escapeStr($val), $specialChars);
            }
          }
          if (!isset($lastFields)) {
    ...
    if (isset($fields) && is_array($fields)) {
      foreach ($fields as $field) {
        if ($field['autoinc'] == 'yes') {
          $tableName = $this->escapeStr($table);
          $fieldName = $this->escapeStr($field['name']);
    ...
    if (is_array($tableData['fields']) && trim($tableData['name']) != '') {
      ...
      if (isset($tableData['keys']) && is_array($tableData['keys'])) {
        if (isset($tableData['keys']['PRIMARY'])) {
          $sql .= 'CONSTRAINT '.$this->escapeStr(strtolower($table)).
        ...
        foreach ($tableData['keys'] as $keyName => $key) {
          if ($keyName != 'PRIMARY' &&
            ...
            $sql .= 'CONSTRAINT '.$this->escapeStr(strtolower($table)).'_'.
              $keyName.' UNIQUE ';
  ...
  function getFieldInfo($table, $fieldName) {
    $sql = "SELECT a.attname AS fieldname,
             ...
             WHERE d.adrelid = a.attrelid
              ...
              FROM pg_class c, pg_attribute a, pg_type t
             WHERE relkind = 'r'
               AND c.relname='".$this->escapeStr($table)."'
               AND a.attname='".$this->escapeStr($fieldName)."'
               AND a.attnum > 0
               AND a.atttypid = t.oid
               AND a.attrelid = c.oid
             ORDER BY a.attnum";
  ...
  */
  function changeField($table, &$fieldData) {
    $result = TRUE;
    $tableName = $this->escapeStr($table);
    $fieldName = $this->escapeStr($fieldData['name']);
    ...
    if ($databaseField = $this->getFieldInfo($table, $fieldData['name'])) {
      ...
      $sql = "ALTER TABLE ".$this->escapeStr($table)."
                ADD COLUMN ".$this->escapeStr($fieldData['name'])." ".$xmlType;
      return ($this->query($sql) !== FALSE);
    }
  }
  ...
  */
  function getIndexInfo($table, $key) {
    $result = FALSE;
    $tableName = $this->escapeStr($table);
    $keyName = ($key == 'PRIMARY') ? 'primary_key' : $this->escapeStr($key);
  ...
  function changeIndex($table, &$index, $dropCurrent = TRUE) {
    ...
    if (isset($index['fields']) && is_array($index['fields'])) {
      ...
      if ($index['name'] == 'PRIMARY') {
        ...
        $sql = 'CREATE UNIQUE INDEX '.$this->escapeStr($table).'_'.
          $this->escapeStr($index['name']).' ON '.$this->escapeStr($table).' '.
            $fields;
      } else {
        $sql = 'CREATE INDEX '.$this->escapeStr($table).'_'.
          $this->escapeStr($index['name']).' ON '.
          $this->escapeStr($table).' '.$fields;
      }
      return ($this->query($sql) !== FALSE);
    }
    return FALSE;
  ...
  */
  function dropIndex($table, $name) {
    if ($key = $this->getIndexInfo($table, $name)) {
      $keyName = $this->escapeStr($key['orgname']);
      if ($key['name'] == 'PRIMARY') {
