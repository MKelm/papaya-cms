...
* @subpackage Database
* @version $Id: mysql.php 38333 2013-03-27 15:04:01Z weinert $
*/
...
class dbcon_mysql extends dbcon_base {
  ...
  */
  function insertRecord($table, $idField, $values = NULL) {
    ...
    if (isset($values) && is_array($values) && count($values) > 0) {
      ...
      $sql = 'INSERT INTO '.$this->escapeStr($table).' ('.$fieldString.')
              VALUES ('.$valueString.')';
  ...
  */
  function addField($table, &$fieldData) {
    $sql = "ALTER TABLE `".$this->escapeStr($table)."` ADD COLUMN `".
          $this->escapeStr($fieldData['name'])."` ".
  ...
  */
  function dropField($table, $field) {
    $sql = "ALTER TABLE `".$this->escapeStr($table)."` DROP COLUMN `".
      $this->escapeStr($field)."`";
    return ($this->query($sql) !== FALSE);
  }
    ...
    if (isset($index['fields']) && is_array($index['fields'])) {
      ...
      if ($res = $this->query($sql)) {
        ...
        if ($indb >= $needed) {
          ...
          $drop = ($dropCurrent) ?
            " DROP INDEX `".$this->escapeStr($index['name'])."`," : '';
