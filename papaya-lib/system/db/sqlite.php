...
* @subpackage Database
* @version $Id: sqlite.php 38333 2013-03-27 15:04:01Z weinert $
*/
...
class dbcon_sqlite extends dbcon_base {
  ...
  */
  function insertRecord($table, $idField, $values = NULL) {
    ...
    if (isset($values) && is_array($values) && count($values) > 0) {
      ...
      $sql = 'INSERT INTO '.$this->escapeStr($table).
  ...
  */
  function queryTableStructure($tableName, $tablePrefix = '') {
    ...
    $sql = "PRAGMA table_info('".$this->escapeStr($table)."');";
    if ($res = $this->query($sql)) {
    ...
    $sql = "PRAGMA index_list('".$this->escapeStr($table)."')";
    if ($res = $this->query($sql)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
      ...
      foreach ($keys as $keyName => $keyData) {
        $sql = "PRAGMA index_info('".$this->escapeStr($keyData['orgname'])."')";
        if ($res = $this->query($sql)) {
          while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
  ...
  function changeIndex($table, &$index, $dropCurrent = TRUE) {
    ...
    if (isset($index['fields']) && is_array($index['fields'])) {
      ...
      if ($index['name'] == 'PRIMARY') {
        $sql = "ALTER TABLE ".$this->escapeStr($table)." ADD PRIMARY KEY ".$fields;
  ...
  * @TODO check if RANDOM() really works with all versions
  */
