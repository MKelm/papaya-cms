...
* @version $Id: papaya_tags.php 38361 2013-04-04 12:09:41Z hapke $
*/
...
class papaya_tags extends base_tags {
  ...
  * php 4 constructor
  */
  function papaya_tags($paramName = 'tg') {
  ...
  */
  function initialize() {
    ...
    $this->layout->setParam('COLUMNWIDTH_CENTER', '250px');
  }

  /**
  * executes tag editing commands
  *
  * @todo check dialog inputs
  */
  function execute() {
      ...
      case 'del_category' :
        if (isset($this->userPermissions['user_edit_category'])) {
          if (isset($this->params['confirm']) && $this->params['confirm'] &&
            ...
            $this->dialog = $this->getDelCategoryDialog();
          }
        }
        break;
      case 'cut_category':
      ...
      case 'del_tag':
        if (count($this->currentTag) > 0
          ...
          if (isset($this->params['confirm']) && $this->params['confirm']
            ...
            $this->dialog = $this->getDelTagDialog();
          }
        }
        break;
      case 'open_category':
  ...
  */
  function loadMenuXML() {
    if ($this->authUser->hasPerm(PapayaAdministrationPermissions::TAG_CATEGORY_MANAGE) &&
    ...
    if (isset($this->categories) && is_array($this->categories)
        ...
        && isset($this->userPermissions['user_edit_tag'])
        && $this->authUser->hasPerm(PapayaAdministrationPermissions::TAG_EDIT)) {
  ...
  */
  function loadCategories() {
    ...
    $lngCondition = $this->databaseGetSQLCondition(
      'ct.lng_id', $this->lngSelect->currentLanguageId
    );
    $parentCondition = $this->databaseGetSQLCondition('c.parent_id', $parentIds);
    $availableCategories = $this->getAvailableCategories(
      $this->authUser, array('user_edit_category', 'user_edit_tag')
    );
        ...
        if ($tag['tag_title'] != '') {
          ...
          if (!(
            ...
            $this->alternativeTags = $this->getTags(
              array_keys($tags), PAPAYA_CONTENT_LANGUAGE
            );
          }
      ...
      foreach ($groups as $groupId => $groupTitle) {
        if (isset($categoryPerms['user_edit_category'][$groupId])) {
          if ($categoryPerms['user_edit_category'][$groupId] == 'own') {
            $createCategoryIcon = $this->images['status-node-checked'];
            ...
            $createCategoryIcon = $this->images['status-node-checked-disabled'];
            $createCategoryLink = '';
          }
        } else {
          if ($this->category['permission_mode'] != 'inherited') {
            $createCategoryIcon = $this->images['status-node-empty'];
            $createCategoryLink = $this->getLink(
              array(
                'cmd' => 'add_perm',
                'cat_id' => $this->category['category_id'],
            ...
            $createCategoryIcon = $this->images['status-node-empty-disabled'];
            $createCategoryLink = '';
          }
        }
        if (isset($categoryPerms['user_edit_tag'][$groupId])) {
          if ($categoryPerms['user_edit_tag'][$groupId] == 'own') {
            $createTagIcon = $this->images['status-node-checked'];
            ...
            $createTagIcon = $this->images['status-node-checked-disabled'];
            $createTagLink = '';
          }
        } else {
          if ($this->category['permission_mode'] != 'inherited') {
            $createTagIcon = $this->images['status-node-empty'];
            $createTagLink = $this->getLink(
              array(
                'cmd' => 'add_perm',
                'cat_id' => $this->category['category_id'],
            ...
            $createTagIcon = $this->images['status-node-empty-disabled'];
            $createTagLink = '';
          }
        }
        if (isset($categoryPerms['user_use_tags'][$groupId])) {
          if ($categoryPerms['user_use_tags'][$groupId] == 'own') {
            $useTagIcon = $this->images['status-node-checked'];
            ...
            $useTagIcon = $this->images['status-node-checked-disabled'];
            $useTagLink = '';
          }
        } else {
          if ($this->category['permission_mode'] != 'inherited') {
            $useTagIcon = $this->images['status-node-empty'];
            $useTagLink = $this->getLink(
              array(
                'cmd' => 'add_perm',
                'cat_id' => $this->category['category_id'],
            ...
            $useTagIcon = $this->images['status-node-empty-disabled'];
            $useTagLink = '';
          }
        }
        $result .= sprintf(
  ...
  */
  function addNewCategory() {
    $lngData = array(
      $this->lngSelect->currentLanguageId => array(
  ...
  */
  function importTags() {
    include_once(PAPAYA_INCLUDE_PATH.'system/sys_csv.php');
    $csvObj = sys_csv::getInstance();
    $associative = FALSE;
    ...
    if ($csvData) {
      ...
      if ($this->categoryIsEmpty($this->params['cat_id'])) {
        ...
        foreach ($tagRecords as $i => $lngData) {
    ...
    foreach ($tagLngData as $lngIdent => $tagData) {
      foreach ($tagData as $level => $title) {
        ...
        if ($title != '') {
          ...
          foreach ($this->lngSelect->languages as $lngId => $lngData) {
            if ($lngData['lng_ident'] == strtolower($lngIdent)) {
              $tagLngId = $lngData['lng_id'];
              break;
            }
          }
  ...
  function addImportTagCategories($parentId, $categoryData, $level = 0) {
    if (isset($categoryData[$level])) {
      ...
      foreach ($categoryData[$level] as $lngId => $category) {
        if ($categoryIds = $this->getCategoryIdsByTitle(
  ...
  */
  function getCacheFileName($cacheId = NULL) {
    ...
    return PAPAYA_PATH_CACHE.'.tags_'.$cacheId.'.csv';
  }
