...
* @version $Id: base_topictree.php 38360 2013-04-04 10:47:15Z weinert $
*/
...
class base_topic_tree extends base_db {
  ...
  */
  function execute() {
    if (!(
          $this->topicId &&
          $this->hasParent($this->authUser->user['start_node'])
        )) {
      if ($this->params['page_id'] != $this->authUser->user['start_node']) {
        ...
        if ($this->authUser->user['start_node'] > 0) {
          $toUrl = $protocol."://".$_SERVER['HTTP_HOST'].$this->getBasePath().
            $this->getLink(array('page_id'=>$this->authUser->user['start_node']));
          $this->sessionParams['page_id'] = $this->authUser->user['start_node'];
        } else {
    ...
    if (isset($this->params['cmd'])) {
      ...
      case 'close':
        ...
        break;
      case 'openall':
        $this->openAll((int)$this->params['page_id']);
        break;
      case 'regenerate':
        if ($this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_REPAIR_INDEX)) {
          $this->regeneratePath();
        }
        break;
      case 'refreshpages':
        if ($this->authUser->hasPerm(PapayaAdministrationPermissions::SYSTEM_CACHE_CLEAR)) {
          $this->refreshPages();
        }
        break;
      }
  ...
  */
  function load($baseId, $lngId) {
    ...
    if (!$this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_TRASH_MANAGE)) {
  ...
  * Open all
  *
  * @param integer $id optional, default value 0
  * @access public
  */
  function openAll($id = 0) {
    if ($id == 0) {
      $sql = "SELECT DISTINCT(prev)
                FROM %s";
      ...
      $sql = "SELECT prev_path, prev
                FROM %s
               WHERE topic_id = '%d'";
      $params = array($this->tableTopics, $id);
      if ($res = $this->databaseQueryFmt($sql, $params)) {
        if ($temp = $res->fetchRow()) {
          if ($prev > 0) {
            $prevPath = str_replace(';;', ';', ';'.$temp[0].$temp[1].';');
          } else {
            $prevPath = str_replace(';;', ';', ';0;'.$temp[1].';'.$id.';');
          }
          $sql = "SELECT topic_id
                  FROM %s
                 WHERE (prev = '%s') OR (prev_path LIKE '%s%%')";
          $params = array($this->tableTopics, $id, $prevPath);
          if ($res = $this->databaseQueryFmt($sql, $params)) {
            while ($row = $res->fetchRow()) {
              $this->opened[$row[0]] = TRUE;
            }
  ...
  *
  * @param interger $baseId ID of Top Entry
  * @access public
  ...
  */
  function topicEditable($topicId) {
    if (isset($this->topics[$topicId])) {
      ...
      if ($this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_MANAGE)) {
        if ($this->authUser->subLevel == 0) {
          return TRUE;
        }
        //Indent level ok
        if (isset($topic['ALLPREVS']) && is_array($topic['ALLPREVS'])) {
          return (count($topic['ALLPREVS']) <= $this->authUser->subLevel);
        } else {
          return TRUE;
        }
      }
