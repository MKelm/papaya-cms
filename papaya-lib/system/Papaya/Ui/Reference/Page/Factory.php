...
* @subpackage Ui
* @version $Id: Factory.php 38272 2013-03-13 13:44:02Z weinert $
*/
...
class PapayaUiReferencePageFactory extends PapayaObject {
  ...
  public function getDomainData($languageIdentifier, $pageId) {
    ...
    if ($pageData = $this->getPageData($languageIdentifier, $pageId)) {
      if (isset($pageData['domain'])) {
        ...
        if ($current && isset($domains[$current['id']])) {
          // given page is accessible on current domain
          $result = TRUE;
        } else {
          $filtered = array_filter($domains, array($this, 'isDomainWithoutWildcards'));
          if (count($filtered) == 1) {
            // given page is accessible on a single other fixed domain (without wildcards)
  ...
  */
  private function setLinkPopupOption($bitmask, $bit, $options, $name, $default = FALSE) {
    if (PapayaUtilArray::get($options, $name, FALSE)) {
      $bitmask &= $bit;
    }
    return $bitmask;
  }
