...
* @version $Id: Teasers.php 38477 2013-05-07 14:27:34Z weinert $
*/
...
class PapayaUiContentTeasers extends PapayaUiControl {
  ...
  */
  ...
  public function appendTo(PapayaXmlElement $parent) {
    ...
    $guids = array();
    $this->papaya()->plugins->preload(
      iterator_to_array(new PapayaIteratorArrayMapper($this->pages(), 'module_guid'))
    );
  ...
  private function appendTeaser(PapayaXmlElement $parent, $record) {
    if (!empty($record['module_guid'])) {
      ...
      if ($plugin) {
        ...
        if ($teaser->ownerDocument->xpath()->evaluate('count(node())', $teaser) == 0) {
