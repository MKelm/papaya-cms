...
* @subpackage Theme
* @version $Id: Wrapper.php 38162 2013-02-23 11:53:39Z weinert $
*/
...
class PapayaThemeWrapper extends PapayaObject {
  ...
  */
  public function getResponse() {
    ...
    if ($application->options->getOption('PAPAYA_CACHE_THEMES', FALSE)) {
      $cacheTime = $application->options->getOption('PAPAYA_CACHE_TIME_THEMES', 0);
    } else {
      $cacheTime = 0;
    }
  ...
  *
  * The identifier are basically files names, but the can omit the extension. In this case the
  ...
  */
  public function getCacheIdentifier($themeSetId, $files, $mimetype, $compress = FALSE) {
    ...
    $result .= implode(
      '_',
      array(
        $options->getOption('PAPAYA_WEBSITE_REVISION', 'dev'),
