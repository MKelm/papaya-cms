...
* @package Papaya
* @subpackage Core
* @version $Id: papaya_modulemanager.php 38271 2013-03-13 13:06:30Z weinert $
*/
...
class papaya_modulemanager extends base_db {
  ...
  */
  var $modulesParamNames = array('type', 'guid', 'name', 'class', 'file');
  /**
  * Module types
  ...
  */
  function getModulesPath() {
    $path = realpath(PAPAYA_INCLUDE_PATH);
    return $this->checkTrailingSlash(str_replace('\\', '/', $path.'/'.$this->modulesPath));
  }
  ...
  function execute() {
    ...
    if (isset($this->params['cmd'])) {
      ...
      case 'scan':
        ...
        $this->searchModules($this->getModulesPath());
      ...
      case 'pkg_table_create':
        ...
        if (isset($this->params['pkg_table_create_confirm']) &&
          ...
          $relPath = $this->packages[$pkgId]['modulegroup_path'];
          $path = $this->getModulesPath().$relPath;
          if (isset($this->tables) &&
            ...
            foreach ($this->tables as $tableName => $inDatabase) {
              if ($inDatabase === TRUE) {
                ...
                $xmlFileName = $path.'DATA/table_'.$tableName.'.xml';
                if ($struct = $this->loadTableStructure($xmlFileName, $pkgId)) {
                  ...
                  if ($created) {
                    ...
                    $this->setTableStatus($relPath, $tableName, FALSE);
                  } else {
                    $this->addMsg(
                      MSG_ERROR,
      ...
      case 'table_create':
        if (isset($this->params['table']) &&
          ...
          $path = $this->getModulesPath().$relPath;
          $xmlFileName = $path.'DATA/table_'.$this->params['table'].'.xml';
          if ($struct = $this->loadTableStructure($xmlFileName, $pkgId)) {
            $created = $this->databaseCreateTable(
              $struct,
      ...
      case 'field_update':
        if (isset($this->params['field_action_confirm']) &&
            ...
            checkit::isAlphaNumChar($this->params['table'])) {
          if (isset($this->packages[$this->params['pkg_id']])) {
            $relPath = $this->packages[$this->params['pkg_id']]['modulegroup_path'];
          }
          $path = $this->getModulesPath().$relPath;
          $xmlFileName = $path.'DATA/table_'.$this->params['table'].'.xml';
      ...
      case 'field_delete':
        if (isset($this->params['field_action_confirm']) &&
            ...
            checkit::isAlphaNumChar($this->params['field'])) {
      ...
      case 'index_delete':
        if (isset($this->params['index_action_confirm']) &&
            ...
            checkit::isAlphaNumChar($this->params['index'])) {
      ...
      case 'export_table':
        if ($this->params['table'] != '') {
          include_once(PAPAYA_INCLUDE_PATH.'system/base_db2xml.php');
  ...
  */
  function rpcCheckTableStatus() {
    ...
    if (isset($package)) {
      ...
      $this->loadTables = TRUE;
      $relPath = $this->packages[$pkgId]['modulegroup_path'];
      $packageFileName = $this->getModulesPath().$relPath.$this->modulesFileName;
      if ($packageData = $this->loadPackageFile($packageFileName, $relPath)) {
  ...
  */
  function getXML() {
    $this->layout->setParam('COLUMNWIDTH_LEFT', '200px');
    $this->layout->setParam('COLUMNWIDTH_CENTER', '400px');
    ...
    if (isset($_GET['module']) && preg_match('/^[a-fA-F\d]{32}$/D', $_GET['module'])) {
      ...
      if ($res = $this->databaseQueryFmt($sql, array($tableModules, $_GET['module']))) {
        if ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
          if (isset($_GET['src']) &&
              preg_match('~^((?:script)/)([\w-]+\.(js|vbs|css|html))$~D', $_GET['src'], $regs)) {
            $scriptPath = $this->getModulesPath().$row['module_path'].'script/';
            ...
            $imagePath = $this->getModulesPath().$row['module_path'].'flash/';
            $imageFiles = array(
              $imagePath.$imageFileName
            );
          } else {
            $imagePath = $this->getModulesPath().$row['module_path'].'pics/';
  ...
  */
  function updatePackageTable() {
    if (isset($this->packages) && is_array($this->packages)) {
      if (FALSE !== $this->databaseEmptyTable(
  ...
  */
  function searchModules($path) {
    ...
    if (file_exists($path)) {
      ...
      $this->addMsg(MSG_ERROR, $this->_gt('No modules directory found.'));
    }
  }
  ...
  */
  function scanDirectory($path) {
    ...
    if (file_exists($packageFileName)) {
      $relPath = substr($path, strlen($this->baseDirectory));
      $this->packages[$relPath] = $this->loadPackageFile($packageFileName, $relPath);
      $this->packages[$relPath]['modulegroup_path'] = $relPath;
    }
    if ($dh = opendir($path)) {
   ...
   * @return void
   */
  public function loadPackageWithDataFromDirectory($baseDirectory, $path) {
    $this->packages = array();
    $this->modules = array();
    $i = 1;
    $packageFileName = $baseDirectory.$path.$this->modulesFileName;
    $this->loadTables = TRUE;
    $info = $this->loadPackageFile($packageFileName, $path);
    $this->packages[$i]['modulegroup_id'] = $i;
    $this->packages[$i]['modulegroup_path'] = $path;
  ...
  */
  function loadPackageFile($packageFileName, $relPath) {
    if (file_exists($packageFileName)) {
      $packageData = array(
        'modulegroup_title' => '',
        'description' => '',
      ...
      $xmlTree = &simple_xmltree::createFromXML(file_get_contents($packageFileName), $this);
      if (isset($xmlTree) && isset($xmlTree->documentElement) &&
        ...
        for ($idx = 0; $idx < $xmlTree->documentElement->childNodes->length; $idx++) {
          ...
          case 'name':
            $packageData['modulegroup_title'] = $node->valueOf();
            break;
          case 'description':
            $packageData['modulegroup_description'] = $node->valueOf();
            break;
          case 'author':
            $packageData['modulegroup_authors'][] = $node->valueOf();
            break;
          case 'prefix':
            $packageData['modulegroup_prefix'] = $node->valueOf();
            break;
          case 'version':
            $packageData['modulegroup_version'] = $node->valueOf();
            break;
          case 'url':
            $packageData['modulegroup_url'] = $node->valueOf();
            break;
          case 'modules':
            if ($node->nodeType == XML_ELEMENT_NODE && $node->hasChildNodes()) {
              for ($idx2 = 0; $idx2 < $node->childNodes->length; $idx2++) {
                $moduleNode = &$node->childNodes->item($idx2);
                if ($moduleNode->nodeType == XML_ELEMENT_NODE &&
                    $moduleNode->nodeName == 'module') {
                  $allParams = TRUE;
                  $moduleData = array(
                    'type' => '',
                    'guid' => '',
                  ...
                  if ($allParams &&
                      in_array($moduleData['type'], $this->moduleTypes)) {
                    $moduleData['description'] = $moduleNode->valueOf();
                    $moduleData['path'] = $relPath;
                    if (isset($this->modules[$moduleData['guid']])) {
                      $this->addMsg(
                        MSG_WARNING,
                        sprintf(
                          $this->_gt(
                            'Duplicate module guid "%s" for "%s" in "%s",'.
                            ' already used for "%s" in "%s".'
                          ),
                          $moduleData['guid'],
                          $moduleData['class'],
                          $moduleData['path'].$moduleData['file'],
                          $this->modules[$moduleData['guid']]['class'],
                          $this->modules[$moduleData['guid']]['path'].
                            $this->modules[$moduleData['guid']]['file']
                        )
                      );
                    } else {
            ...
            break;
          case 'tables':
            if ($this->loadTables && $node->hasChildNodes()) {
              for ($idx2 = 0; $idx2 < $node->childNodes->length; $idx2++) {
                $tableNode = &$node->childNodes->item($idx2);
                if ($tableNode->nodeType == XML_ELEMENT_NODE &&
                    $tableNode->nodeName == 'table' &&
                    $tableNode->hasAttribute('name')) {
        ...
        $this->addMsg(
          ...
          sprintf(
            ...
            papaya_strings::escapeHTMLChars($relPath.basename($packageFileName))
          )
        );
      }
    }
  ...
  */
  function loadPackages() {
    unset($this->packages);
    $sql = "SELECT modulegroup_id, modulegroup_title, modulegroup_path, modulegroup_prefix
              FROM %s
  ...
  */
  function loadPackageData($pkgId) {
    ...
    if (isset($this->packages[$pkgId])) {
      ...
      $packageFileName = $this->getModulesPath().$relPath.$this->modulesFileName;
      $this->loadTables = TRUE;
      $package = $this->loadPackageFile($packageFileName, $relPath);
  ...
  */
  function loadTable($pkgId, $table) {
    if (isset($table) && trim($table) != '' &&
      ...
      $relPath = $package['modulegroup_path'];
      $absPath = $this->getModulesPath().$relPath;
      $tableFile = $absPath.'DATA/table_'.$table.'.xml';
      ...
      if (isset($this->tableStruct['name']) &&
        ...
        if (isset($this->tableStruct['actions']) && $this->tableStruct['actions'] > 0) {
          $this->setTableStatus($relPath, $table, TRUE);
        } else {
          $this->setTableStatus($relPath, $table, FALSE);
        }
      } else {
        $this->setTableStatus($relPath, $table, FALSE, TRUE);
      }
    }
  }
  ...
  */
  function loadTableStructure($xmlFileName, $pkgId = NULL) {
    if (file_exists($xmlFileName)) {
      ...
      $tableData = array();
      include_once(PAPAYA_INCLUDE_PATH.'system/sys_simple_xmltree.php');
      $xmlTree = &simple_xmltree::createFromXML(file_get_contents($xmlFileName), $this);
      if (isset($xmlTree) && isset($xmlTree->documentElement) &&
          $xmlTree->documentElement->hasChildNodes()) {
            ...
            case 'keys':
              if ($node->hasChildNodes()) {
                for ($idx2 = 0; $idx2 < $node->childNodes->length; $idx2++) {
                  ...
                  if ($keyNode->nodeType == XML_ELEMENT_NODE) {
                    ...
                    case 'key':
                      ...
                      if ($keyNode->hasChildNodes()) {
                        for ($idx3 = 0; $idx3 < $keyNode->childNodes->length; $idx3++) {
                          ...
                          if ($fieldNode->nodeType == XML_ELEMENT_NODE &&
                              $fieldNode->nodeName == 'field') {
                            $s = strtolower($fieldNode->valueOf());
  ...
  function getPackageListView() {
    if (isset($this->packages) && is_array($this->packages)) {
      ...
      foreach ($this->packages as $package) {
        ...
        $packageStatus = $this->getPackageStatus($package['modulegroup_path']);
    ...
    if (isset($this->params['module_id']) && isset($this->modules[$this->params['module_id']])) {
      ...
      if (isset($this->module) && isset($this->module['module_title'])) {
        ...
        $result .= sprintf(
          ...
          papaya_strings::escapeHTMLChars($this->module['module_path']),
          papaya_strings::escapeHTMLChars($this->module['module_file'])
        );
      } else {
        $result .= sprintf(
          '<line class="headertitle">%s</line>',
        ...
        foreach ($this->modules as $module) {
          if (isset($module) && is_array($module)) {
            ...
            if (isset($module['error']) && $module['error']) {
              $glyph = $this->images['status-dialog-error'];
            } elseif (trim($module['glyph']) != '') {
              $glyph = './modglyph.php?module='.urlencode($module['guid']);
            } else {
              switch ($module['type']) {
              case 'alias':
                $glyph = $this->images['items-alias'];
                break;
              case 'page':
                $glyph = $this->images['items-page'];
                break;
              case 'box':
                $glyph = $this->images['items-box'];
                break;
              case 'admin':
                $glyph = $this->images['items-option'];
                break;
              case 'cronjob':
                $glyph = $this->images['items-cronjob'];
                break;
              case 'time':
                $glyph = $this->images['items-time'];
                break;
              case 'date':
                $glyph = $this->images['items-date'];
                break;
              case 'output':
                $glyph = $this->images['items-filter-export'];
                break;
              case 'import':
                $glyph = $this->images['items-filter-import'];
                break;
              case 'image':
                $glyph = $this->images['items-graphic'];
                break;
              case 'parser':
                $glyph = $this->images['items-plugin'];
                break;
              case 'statistic':
                $glyph = $this->images['items-statistic'];
                break;
              case 'datafilter':
                $glyph = $this->images['items-filter-convert'];
                break;
              default:
                $glyph = $this->images['items-plugin'];
              }
            ...
            if (isset($module['active']) && $module['active']) {
              ...
              $activeGlyph = $this->images['status-node-checked'];
            } else {
              $href = $this->getLink(
                array('cmd' => 'module_enable', 'module_id' => $module['guid'])
              );
              $activeGlyph = $this->images['status-node-empty'];
            }
            $str .= sprintf(
      ...
      if ($showModules) {
        ...
        foreach ($this->tables as $tableName => $inDatabase) {
          if (isset($this->params['pkg_id']) &&
              ...
              $inDatabase) {
            $modulePath = $this->packages[$this->params['pkg_id']]['modulegroup_path'];
          ...
          $strItem = sprintf(
            ...
            $selected);
          $activeGlyph = papaya_strings::escapeHTMLChars($this->images[$imageIdx]);
  ...
  function exportTableData() {
    if ($this->params['table'] != '') {
      ...
      if ($res) {
        ...
        $this->csvFile = base_csv::getInstance();
        if ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
          $fileName = $tableName.'_'.date('Y-m-d').'.csv';
          $this->csvFile->outputExportHeaders($fileName);
          $keys = array_keys($row);
  ...
  function initModuleDialog() {
    if (!(isset($this->moduleDialog) && is_object($this->moduleDialog))) {
      if (isset($this->module) && is_array($this->module)) {
        ...
        $this->moduleDialog->msgs = &$this->msgs;
  ...
  */
  function getPackageDialog() {
    if ((isset($this->module) && is_array($this->module)) ||
      ...
      $missing = FALSE;
      if (isset($this->tables)) {
        foreach ($this->tables as $table => $inDatabase) {
          if ($inDatabase !== TRUE) {
            $missing = TRUE;
            break;
          }
    ...
    if (isset($struct['fields']) && is_array($struct['fields'])) {
      ...
      foreach ($struct['fields'] as $field) {
        ...
        if (isset($field['action']) && $field['action']) {
          switch ($field['action']) {
          case 'add':
            $glyph = $this->images['actions-generic-add'];
            break;
          case 'delete':
            $glyph = $this->images['actions-generic-delete'];
            break;
          case 'update':
            $glyph = $this->images['actions-edit'];
            break;
          default :
            $glyph = $this->images[0];
          }
  ...
  function setTableStatus($packagePath, $tableName, $modified = FALSE, $missing = FALSE) {
    if ($missing) {
      ...
      if (isset($this->sessionParams['missing_tables'][$packagePath]) &&
        ...
        foreach ($this->sessionParams['missing_tables'][$packagePath] as $table => $status) {
          if ($status) {
      ...
      if (isset($this->sessionParams['modified_tables'][$packagePath]) &&
        ...
        foreach ($this->sessionParams['modified_tables'][$packagePath] as $table => $status) {
          if ($status) {
  ...
  */
  function getPluginOptionsDialog() {
    if (isset($this->module)) {
      ...
      $pluginObject = &base_pluginloader::getPluginInstance(
  ...
  */
  function initTableImportDialog() {
    if (!(isset($this->dialogTableImport) && is_object($this->dialogTableImport))) {
      ...
      $this->dialogTableImport->msgs = &$this->msgs;
  ...
  */
  function getDefaultTableDataFile($pkgId, $table) {
    $relPath = $this->packages[$pkgId]['modulegroup_path'].'DATA/';
    $path = $this->getModulesPath().$relPath;
