...
* @version $Id: base_language_select.php 36477 2011-12-03 13:25:26Z weinert $
*/
...
class base_language_select extends base_db {
  ...
  */
  public function __get($name) {
    switch ($name) {
    case 'currentLanguageId' :
      return $this->papaya()->administrationLanguage->getCurrent()->id;
    case 'currentLanguage' :
  ...
  */
  public function getContentLanguageCombo($paramName, $name, $element, $data,
                                   $emptyElement = NULL, $nameOnly = FALSE) {
    $result = '';
  ...
  */
  public function getContentLanguageLinksXML() {
    return $this->papaya()->administrationLanguage->getXml();
  }
  ...
  */
  public function getCurrentLanguageIcon() {
    $image = $this->papaya()->administrationLanguage->getCurrent()->image;
    if (empty($image)) {
      return '';
    } else {
  ...
  */
  public function getCurrentLanguageTitle($separator = ' - ') {
    $title = $this->papaya()->administrationLanguage->getCurrent()->title;
    if (empty($title)) {
      return '';
    } else {
