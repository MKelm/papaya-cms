...
* @package Papaya
* @subpackage Core
* @version $Id: base_cron.php 36765 2012-02-24 14:13:46Z kelm $
*/
...
class base_cronjobs extends base_db {
  ...
  */
  function check() {
    ...
    if (empty($this->params['cronjob_title']) ||
        !checkit::isNoHTML($this->params['cronjob_title'], TRUE)) {
      $result = FALSE;
      $this->addMsg(
        MSG_ERROR,
    ...
    if (empty($this->params['cronjob_description']) ||
        !checkit::isNoHTML($this->params['cronjob_description'], TRUE)) {
      $result = FALSE;
      $this->addMsg(
        MSG_ERROR,
    ...
    if (empty($this->params['cron_module_guid']) ||
        !checkit::isGUID($this->params['cron_module_guid'], TRUE)) {
      $result = FALSE;
      $this->addMsg(
        MSG_ERROR,
    ...
    if (empty($this->params['job_module_guid']) ||
        !checkit::isGUID($this->params['job_module_guid'], TRUE)) {
      $result = FALSE;
      $this->addMsg(
        MSG_ERROR,
    ...
    $this->params['cronjob_start'] =
      checkit::convertISODateTimeToUnix($this->params['cronjob_start_str']);
    $this->params['cronjob_end'] =
      checkit::convertISODateTimeToUnix($this->params['cronjob_end_str']);
    ...
    case 'activate' :
      if (isset($this->params['id'])) {
        ...
        if ($dialog->checkDialogInput()) {
          $nextExecutionTime = checkit::convertISODateTimeToUnix($dialog->data['nextexec']);
  ...
  function executeJob($id, $verbose = TRUE) {
    ...
    if ($this->load($id)) {
      ...
      if ($this->cronjob['cronjob_nextexec'] > 0) {
        ...
        $jobModule = &base_pluginloader::getPluginInstance(
  ...
  function getListXML() {
    if (isset($this->cronjobs) && is_array($this->cronjobs)) {
      ...
      foreach ($this->cronjobs as $cronjobId => $cronjob) {
        $nextexecStr = ($cronjob['cronjob_nextexec'] > time())
          ? PapayaUtilDate::timestampToString($cronjob['cronjob_nextexec'], FALSE)
          : $this->_gt('now');
  ...
  function initializePropertyDialog() {
    if (isset($this->cronjob) && is_array($this->cronjob)) {
      if (!(isset($this->propertyDialog) && is_array($this->propertyDialog))) {
        ...
        $this->propertyDialog->msgs = &$this->msgs;
  ...
  */
  function checkCronJobData($nextExecutionTime = NULL) {
    include_once(PAPAYA_INCLUDE_PATH.'system/base_pluginloader.php');
    $cronModule = &base_pluginloader::getPluginInstance(
      $this->cronjob['cron_module_guid'],
      $this,
      $this->cronjob['cron_data'],
      $this->cronjob['cron_module_class'],
  ...
  */
  function &initializeActivateForm() {
    ...
    if (isset($this->cronjob) && is_array($this->cronjob) &&
      ...
      $currentNextExec = checkit::convertISODateTimeToUnix($dialog->data['nextexec']);
