...
* @version $Id: papaya_linktypes.php 36979 2012-04-17 08:26:10Z weinert $
*/


/**
* Basic class Linktypes
*/
require_once(PAPAYA_INCLUDE_PATH.'system/base_linktypes.php');
  ...
  */
  function getXML() {
    $this->menubar->addButton(
      'Add linktype',
      $this->getLink(array('cmd'=>'add_linktype')),
      'actions-link-add',
      '',
      FALSE
    );
      ...
      foreach ($this->linkTypes as $linkType) {
        if ($linkType['linktype_id'] < 3) {
          $result .= sprintf(
            ...
            papaya_strings::escapeHTMLChars($this->images['status-link-locked'])
          );
        } else {
          $linkParams = array(
            'cmd' => 'edit_linktype',
  ...
  */
  function getEditPopupPropertiesDialog($cmd) {
    ...
    if ($cmd == 'edit_popup' && isset($this->params['ltid']) && (int)$this->params['ltid'] > 2) {
      ...
      $hidden['ltid'] = $this->params['ltid'];
      $popupConfigXML = $linkType['popup_config'];
      $data = array(
        // 'popup_dependent' => $linkType['popup_config']['dependent'],
        'popup_height' => $linkType['popup_config']['height'],
        'popup_location' => $linkType['popup_config']['location'],
        'popup_menubar' => $linkType['popup_config']['menubar'],
        'popup_resizable' => $linkType['popup_config']['resizable'],
        'popup_left' => $linkType['popup_config']['left'],
        'popup_top' => $linkType['popup_config']['top'],
        'popup_scrollbars' => $linkType['popup_config']['scrollbars'],
        'popup_status' => $linkType['popup_config']['status'],
        'popup_toolbar' => $linkType['popup_config']['toolbar'],
        'popup_width' => $linkType['popup_config']['width'],
      );
      $title = 'Edit popup properties';
    }
  ...
  function setLinktype() {
    if (isset($this->params['ltid']) && (int)$this->params['ltid'] > 2) {
      if (isset($this->params['cmd']) && $this->params['cmd'] == 'edit_popup') {
        ...
        $data = array(
          'linktype_popup_config' => simple_xmltree::serializeArrayToXML('data', $popupConfig)
        );
      } else {
        $data = array(
          'linktype_name' => $this->params['linktype_name'],
