...
class papaya_overview extends base_db {
  ...
  function getXML() {
    ...
    default:
      $this->loadNoPublishedTopicList(
        ...
        TRUE,
        $this->authUser->options['PAPAYA_OVERVIEW_ITEMS_UNPUBLISHED']
      );
      $this->layout->add($this->getTopicsList("Not published", '100%'));
      $this->loadTopicList(
        $this->lngSelect->currentLanguageId,
        TRUE,
        $this->authUser->options['PAPAYA_OVERVIEW_ITEMS_PUBLISHED']
      );
    ...
    $this->loadUsers();
    $this->loadTodoList($this->authUser->options['PAPAYA_OVERVIEW_ITEMS_TASKS']);
    $this->layout->addLeft($this->getTodoList('100%'));
    if ($this->authUser->hasPerm(PapayaAdministrationPermissions::MESSAGES)) {
      $this->loadMessageList(
        TRUE,
        $this->authUser->options['PAPAYA_OVERVIEW_ITEMS_MESSAGES']
      );
  ...
  function getTopicsList($title, $width = 490, $showDetails = FALSE) {
    if (isset($this->topics) && is_array($this->topics) && count($this->topics) > 0) {
      ...
      foreach ($this->topics as $key => $topic) {
        ...
        $item->text = $text;
        $item->emphased = ($topic['user_id'] == $this->authUser->userId);
  ...
  */
  function loadMessageList($desc = TRUE, $max = 8) {
    ...
    $params = array($this->tableMessages, $this->authUser->user['user_id'], $order);
    if ($res = $this->databaseQueryFmt($sql, $params, $max)) {
  ...
  */
  function getMessageElements() {
    ...
    if (isset($this->messageList) && is_array($this->messageList)) {
      foreach ($this->messageList as $id => $values) {
  ...
  */
  function loadTodoList($max = 8) {
    ...
    $params = array($this->tableTodos, $this->authUser->user["user_id"]);
    if ($res = $this->databaseQueryFmt($sql, $params, $max)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
  ...
  */
  function getTodoElements() {
    ...
    if (isset($this->todos) && is_array($this->todos)) {
      foreach ($this->todos as $id=>$values) {
        if (isset($values) && is_array($values)) {
          if ($values['date_to'] > 0 ) {
          ...
          $result .= sprintf(
            ...
            papaya_strings::escapeHTMLChars($this->images[$iconIndex])
          );
  ...
  */
  function initializeSearchDialog() {
    if (!(isset($this->dialogSearch) && is_object($this->dialogSearch))) {
      ...
      $this->dialogSearch->msgs = &$this->msgs;
  ...
  */
  function searchFor($limit = 20) {
    ...
    $dateFrom = empty($this->params['filter_date_from'])
      ? 0 : checkit::convertISODateTimeToUnix($this->params['filter_date_from']);
    $dateTo = empty($this->params['filter_date_to'])
      ? time() + 86400
      : checkit::convertISODateTimeToUnix($this->params['filter_date_to']) + 86400;
    $sqlParams = array(
      $this->tableTopics,
      $this->tableAuthUser,
      $this->tableTopicsTrans,
    ...
    if (isset($this->params['filter_date_option'])) {
      ...
      case 'created':
        $conditions[] = "t.topic_created >= '%d' AND t.topic_created <= '%d'";
        break;
      case 'modified':
        $conditions[] = "t.topic_modified >= '%d' AND t.topic_modified <= '%d'";
        break;
      case 'published':
        $conditions[] = "tp.topic_modified >= '%d' AND tp.topic_modified <= '%d'";
        break;
      }
    }
    if (!empty($conditions)) {
