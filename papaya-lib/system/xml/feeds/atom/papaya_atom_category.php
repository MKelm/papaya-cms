...
* @subpackage XML-Feed
* @version $Id: papaya_atom_category.php 37118 2012-06-11 11:19:33Z weinert $
*/
...
class papaya_atom_category extends papaya_atom_element {
  ...
  */
  function assign($category) {
    if (is_object($category) && is_a($category, 'papaya_atom_category')) {
  ...
  function load($node) {
    if ($node->hasAttribute('term')) {
      ...
      if ($node->hasAttribute('scheme')) {
        $this->schemepe = $node->getAttribute('scheme');
      }
