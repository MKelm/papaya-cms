...
* @subpackage XML-Feed
* @version $Id: papaya_atom_text.php 37118 2012-06-11 11:19:33Z weinert $
*/
...
class papaya_atom_text extends papaya_atom_element {
  ...
  */
  function assign($text) {
    if (is_object($text) && is_a($text, 'papaya_atom_text')) {
  ...
  function load($node) {
    ...
    if ($this->_type == 'xhtml' || $typeEnd == '+xml' || $typeEnd == '/xml') {
      ...
      if ($node->hasChildNodes()) {
        $xmlString = '';
        if (method_exists($node, 'saveXML')) {
          for ($i = 0; $i < $node->childNodes->length; $i++) {
            $childNode = $node->childNodes->item($i);
            $xmlString .= $childNode->saveXML();
          }
        } else {
          for ($i = 0; $i < $node->childNodes->length; $i++) {
            $childNode = $node->childNodes->item($i);
            $xmlString .= $childNode->ownerDocument->saveXML($childNode);
              ...
              $this->_type == 'html' ||
              substr($this->_type, 0, -4) == 'text') {
      //ok it is just text
      if (method_exists($node, 'valueOf')) {
        $this->setValue($node->valueOf());
      } else {
      ...
      if (method_exists($node, 'valueOf')) {
        $this->setValue(base64_decode($node->valueOf()));
      } else {
