...
* @version $Id: papaya_tagselector.php 37744 2012-11-29 15:39:51Z weinert $
*/
...
class papaya_tagselector extends base_tags {
  ...
  */
  function &getInstance(&$parentObj) {
    if (isset($instance) &&
        is_object($instance) &&
        is_a($instance, 'papaya_tagselector')) {
      return $instance;
    } elseif ($parentObj->authUser->hasPerm(PapayaAdministrationPermissions::TAG_MANAGE)
  ...
  *
  * @param array $linkedTags array of tags that are linked, including tag titles, etc.
  ...
  */
  function getLinkedTagsListXML($selectedTags) {
    ...
    foreach ($selectedTags as $tagId => $tag) {
      ...
      $image = $this->images['actions-tag-delete'];
      $result .= sprintf(
  ...
  function getTagLinkerList($caption, $selectedTagId, $lngId) {
    ...
    if (isset($this->categories) && is_array($this->categories)
      ...
      $tags = $this->getTagsByCategory(
        array_keys($this->categories[$this->params['cat_id']]), $lngId
      );
      $result .= sprintf('<listview title="%s">'.LF, $caption);
      $result .= '<items>';
      if (is_array($this->categories) && count($this->categories) > 0) {
        foreach ($this->categories as $catId => $category) {
          if (!isset($this->sessionParams['open_categories'][$catId])) {
            $result .= sprintf(
              '<listitem href="%s" image="%s" title="%s" node="empty"><subitem /></listitem>'.LF,
              papaya_strings::escapeHTMLChars(
                $this->getLink(array('cmd' => 'open_category', 'cat_id' => $catId))
              ),
              papaya_strings::escapeHTMLChars($this->images['status-node-closed']),
              papaya_strings::escapeHTMLChars($category['category_title'])
            );
          } else {
            $result .= sprintf(
              '<listitem href="%s" image="%s" title="%s" node="empty"><subitem /></listitem>'.LF,
              papaya_strings::escapeHTMLChars(
                $this->getLink(array('cmd' => 'close_category', 'cat_id' => $catId))
              ),
              papaya_strings::escapeHTMLChars($this->images['status-node-open']),
              papaya_strings::escapeHTMLChars($category['category_title'])
            );
            if (isset($tags[$catId])) {
              foreach ($tags[$catId] as $tagId => $tag) {
                if ($selectedTagId == $tagId) {
                  $link = '';
                  $subItem = sprintf(
                    '<subitem><glyph src="%s" /></subitem>'.LF,
                    papaya_strings::escapeHTMLChars($this->images['status-tag-linked'])
                  );
                  ...
                  $subItem = sprintf(
                    ...
                    papaya_strings::escapeHTMLChars(
                      $this->getLink(array('cmd' => 'select_tag', 'tag_id' => $tagId))
                    ),
                    papaya_strings::escapeHTMLChars($this->images['items-tag'])
                  );
                  $selected = '';
                ...
                $result .= sprintf(
                  '<listitem indent="1" image="%s" title="%s" %s href="%s">'.LF,
                  papaya_strings::escapeHTMLChars($this->images['items-tag']),
                  papaya_strings::escapeHTMLChars($tag['tag_title']),
                  $selected,
                  papaya_strings::escapeHTMLChars(
    ...
    if (isset($tags) && is_array($tags) && count($tags) > 0) {
      ...
      foreach ($tags as $tag) {
        if (isset($linkedTags[$tag['tag_id']])) {
          ...
          $tagIcon = $this->images['actions-tag-delete'];
          ...
          $tagIcon = $this->images['actions-tag-add'];
  ...
  function getAvailableTagsListXMLForSelector($tags, $selectedTags) {
    ...
    foreach ($tags[$this->params['cat_id']] as $tagId => $tag) {
      if (isset($selectedTags[$tagId])) {
        ...
        $image = $this->images['actions-tag-add'];
        $tagHint = $this->_gt('Link tag');
      }
      $result .= sprintf(
        '<listitem image="%s" title="%s">'.LF,
