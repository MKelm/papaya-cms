...
* @package Papaya
* @subpackage Dialogs
* @version $Id: base_dynform.php 37047 2012-05-11 14:36:09Z weinert $
*/
...
class base_dynamic_form extends base_object {
  ...
  * PHP4 Constructor
  */
  function base_dynamic_form($xml, $data = NULL, $hidden = NULL) {
    $this->__construct($xml, $data, $hidden);
  }
  ...
  */
  function checkDialogInputs($oldData = NULL) {
    ...
    if (isset($this->fields) && is_array($this->fields) &&
        isset($checkFunctions) && is_array($checkFunctions)) {
      $checkFunctions = array_flip($checkFunctions);
      foreach ($this->fields as $field) {
        if ($field['type'] == 'file' || $field['type'] == 'imagefile') {
          ...
          if (base_dialog::checkCaptchaAnswer($answer, $identifier)) {
          ...
          } elseif (isset($checkFunctions[strtolower($field['validatefunc'])])) {
            $checkFunction = $field['validatefunc'];
            if ($checkObj->$checkFunction(
                  $this->params[$field['name']], $field['needed'])
                ) {
              $this->validatedData[$field['name']] = $this->params[$field['name']];
            } else {
              $this->addMsg(
  ...
  *
  * @todo $folder (line 529) is not used -> review
  */
            ...
            if ($node->hasChildNodes()) {
              ...
              for ($idx2 = 0; $idx2 < $node->childNodes->length; $idx2++) {
                ...
                if ($fieldNode->nodeType == XML_ELEMENT_NODE
                  ...
                  $group['legend'] = $fieldNode->valueOf();
                }
                if ($fieldNode->nodeType == XML_ELEMENT_NODE
                    && $fieldNode->nodeName == 'text') {
                  $group['legend-text'] = $fieldNode->valueOf();
                }
                  ...
                  if ($fieldNode->hasAttribute('name')) {
                    ...
                    if ($fieldNode->hasChildNodes()) {
                      for ($idx3 = 0; $idx3 < $fieldNode->childNodes->length; $idx3++) {
                        ...
                        if ($typeNode->nodeType == XML_ELEMENT_NODE && $typeNode->nodeName != '') {
                          ...
                          if ($typeNode->hasChildNodes()) {
                            for ($idx4 = 0; $idx4 < $typeNode->childNodes->length; $idx4++) {
                              ...
                              $field['typeparams'][$paramNode->nodeName] = $paramNode->valueOf();
                            }
                          }
                          break;
                        }
  ...
  * @return string HTML
  *
  * @todo $valueOff (line 685) is not used -> review
  */
  function getFieldXMLCheckbox(&$field, $value, $cssClassName) {
    ...
    $valueOffCheck = isset($field['typeparams']['value_off']) &&
      trim($field['typeparams']['value_off']) != '';
    $valueOff = ($valueOffCheck) ? $field['typeparams']['value_off'] : 'O';
  ...
  *
  * @param array $field
  * @param mixed $value Parameter is not used!!
  * @param string $cssClassName
  * @return string
  *
  * @todo need to be reviewed
  */
  function getFieldXMLCaptcha(&$field, $value, $cssClassName) {
    $result = '';
