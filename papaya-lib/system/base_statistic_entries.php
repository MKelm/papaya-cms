...
* @version $Id: base_statistic_entries.php 37440 2012-08-20 16:50:31Z weinert $
*/
...
class base_statistic_entries extends base_db_statistic {
  ...
  *
  * @param string $uri cache entry uri
  * @param boolean $fuzzy whether fuzzy uris are accepted, i.e. the type
  *   doesn't need to be specified
  * @return array $result FALSE on error, otherwise the timestamp of the latest
  *   cache entry
  */
  ...
  *
  * @todo this seemed to use local time instead of gmtime, check this!
  *
  ...
  */
  function getEntriesForSession($sessionType, $guid, $identifier, $from, $to,
                                $type = '', $offset = 0, $limit = 1000) {
    $result = FALSE;
  ...
  *
  * @param string $uri statistic uri, usually module_guid:stat_type:parameters
  * @param integer $from unix timestamp start of timeframe
  * @param integer $to unix timestamp end of timeframe
  ...
  */
  function storeCachedData($guid, $type, $from, $to, $cacheData,
                           $serialize = TRUE, $delayed = FALSE, $subject = NULL) {
  ...
  */
  function processUncachedRecords(&$parentObj, $entryCounterGUID, $entryType, $cacheType,
                                  $bySession = TRUE) {
  ...
  * @param array $sessionEntries sessions
