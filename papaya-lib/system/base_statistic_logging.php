...
* @version $Id: base_statistic_logging.php 37913 2012-12-27 09:56:52Z weinert $
*/
    ...
    if ($this->loggable($preview, $filename)) {
      ...
      default:
        ...
        if ($statisticPageId = $this->getStatisticPageId($dataPage)) {
          ...
          // we use the nonexistant page id 0, so we have the raw data if need arises
          $statisticPageId = 0;
        }
        $dataRequest['statistic_server_id'] = PAPAYA_WEBSERVER_IDENT;
        if ($cachedPage) {
  ...
  */
  function loggable($preview = FALSE, $filename = '') {
    if (defined('PAPAYA_ADMIN_PAGE') && PAPAYA_ADMIN_PAGE) {
      ...
      $mediaOutputModes = array('media', 'thumbnail', 'image', 'thumb');
