...
* @package Papaya
* @subpackage Core
* @version $Id: papaya_systemtest.php 37048 2012-05-11 14:38:42Z weinert $
*/
...
class papaya_systemtest {
  ...
  */
  function infoMemoryLimits() {
    ...
    if (extension_loaded('suhosin')) {
      ...
      if ($suhosinMemoryLimit > 0) {
        $result .= ' (Suhoshin '.$suhosinMemoryLimit.')';
      }
    }
    return $result;
  }
  ...
  */
  function testDatabaseConnection($title) {
    $database = PapayaApplication::getInstance()->getObject('Database')->getConnector();
    try {
