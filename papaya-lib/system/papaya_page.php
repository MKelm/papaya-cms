...
class papaya_page extends base_object {
  ...
  */
  ...
  /**
  * topic object
  * @var papaya_(public)topic $topic
  */
  var $topic = NULL;
  ...
  * @var boolean
  */
  var $allowSessionRedirect = FALSE;
  /**
  ...
  */
  function execute() {
    ...
    $application->setObject('Response', new PapayaResponse());
    ...
    $options = $application->getObject('Options');
    if (!$options->load()) {
    ...
    if ($this->isPreview()) {
      ...
      if ($options->getOption('PAPAYA_UI_SECURE', FALSE) &&
  ...
  */
  function initializeParams($sessionParamName = NULL) {
    ...
    if (isset($_GET['papaya_box_preview']) && $_GET['papaya_box_preview'] > 0) {
    ...
    if ($status == 404) {
      ...
      if (preg_match('(^/?([\\("]|\\\\"))', $requestURI)) {
        ...
        */
        ...
        if ($themeWrapperUrl->getMimetype()) {
  ...
  */
  function setRequestFromAlias($alias) {
    ...
    $parameterGroupSeparator = $application
      ->options
      ->getOption('PAPAYA_URL_LEVEL_SEPARATOR');
  ...
  */
  function queryStringToArray($queryString) {
    $query = new PapayaRequestParametersQuery(
      $this->papaya()->options->getOption('PAPAYA_URL_LEVEL_SEPARATOR', '')
    );
  ...
  */
  function createTopic() {
    ...
    if ($this->isPreview()) {
      ...
      $this->topic = new papaya_topic;
    } else {
      include_once(PAPAYA_INCLUDE_PATH.'system/papaya_publictopic.php');
      $this->topic = new papaya_publictopic;
    }
  }
  ...
  */
  function getCacheId() {
    $definition = NULL;
    $pagePlugin = $this->papaya()->plugins->get($this->topic->topic['TRANSLATION']['module_guid']);
    if ($pagePlugin && ($boxesList = $this->getBoxes())) {
      $pagePlugin->setData($this->topic->topic['TRANSLATION']['topic_content']);
      if ($pagePlugin instanceOf PapayaPluginCacheable) {
        $definition = $pagePlugin->cacheable();
      } elseif (empty($pagePlugin->cacheable) || $pagePlugin->cacheable == FALSE) {
  ...
  function initPageMode() {
    ...
    if (isset($this->mode) && in_array($this->mode, $pageModes)) {
      ...
      if ($pageStatus->sessionMode == 0) {
        $this->allowSession = $this->papaya()->options->getOption(
            ...
            if ($this->filter = $this->output->getFilter($viewId)) {
              ...
              if ($this->filter->checkConfiguration()) {
                ...
                if ($application->options->getOption('PAPAYA_LOG_RUNTIME_REQUEST', FALSE)) {
                  ...
                  PapayaRequestLog::getInstance()->omit(FALSE);
                }
                $this->sendHeader('X-Papaya-Cache: no');
                if ($this->acceptGzip  &&
                    $this->papaya()->options->get('PAPAYA_COMPRESS_OUTPUT', FALSE)) {
                  if ($this->sendHeader('Content-Encoding: gzip')) {
                    $this->sendHeader('X-Papaya-Gzip: yes');
                    $compressed = gzencode($str);
                    $this->sendHeader('Content-Length: '.strlen($compressed));
                    echo $compressed;
                  } else {
                    $this->sendHeader('X-Papaya-Gzip: no');
                ...
                $this->getError(
                  ...
                  'Output mode "'.
                    ...
                    ' not configured: '.$this->filter->errorMessage,
  ...
  */
  function generatePage($filterParams = NULL, $outputContent = TRUE, $allowRedirect = TRUE) {
    ...
    $this->layout = new papaya_xsl();

    if (isset($this->output->viewMode) && !empty($this->output->viewMode['viewmode_ext'])) {
      $currentViewMode = $this->output->viewMode['viewmode_ext'];
      if ($this->papaya()->options->get('PAPAYA_URL_EXTENSION') != '') {
        $defaultViewMode = PAPAYA_URL_EXTENSION;
      } else {
        $defaultViewMode = 'php';
      }
    } elseif ($this->papaya()->options->get('PAPAYA_URL_EXTENSION') != '') {
      $currentViewMode = $this->papaya()->options->get('PAPAYA_URL_EXTENSION');
      $defaultViewMode = $this->papaya()->options->get('PAPAYA_URL_EXTENSION');
    } else {
      $currentViewMode = 'php';
      $defaultViewMode = 'php';
    }
    ...
    if ($outputContent) {
      ...
      $this->layout->setParam('PAGE_BASE_URL', $url);
      ...
      $this->layout->setParam('PAGE_URL', $url);
      $this->layout->setParam(
        'PAGE_TITLE',
        $this->topic->topic['TRANSLATION']['topic_title']
      );
      if (defined('PAPAYA_WEBSITE_REVISION') && trim(PAPAYA_WEBSITE_REVISION) != '') {
        $this->layout->setParam('PAPAYA_WEBSITE_REVISION', PAPAYA_WEBSITE_REVISION);
        $this->layout->setParam('PAGE_WEBSITE_REVISION', PAPAYA_WEBSITE_REVISION);
      }
      ...
      $this->layout->setParam('PAGE_WEB_PATH', PAPAYA_PATH_WEB);
      $this->layout->setParam(
        'PAGE_LANGUAGE',
        $this->topic->currentLanguage['lng_short']
      );
      $this->layout->setParam('PAGE_MODE_PUBLIC', $this->isPreview() ? 0 : 1);
      $this->layout->setParam('PAGE_OUTPUTMODE_CURRENT', $currentViewMode);
      $this->layout->setParam('PAGE_OUTPUTMODE_DEFAULT', $defaultViewMode);
      $this->layout->setParam(
      ...
      if (empty($_SERVER['HTTP_USER_AGENT']) ||
        ...
        if ($boxesId = $this->topic->getBoxesTopicID()) {
  ...
  public function getBoxes() {
    if (!isset($this->_boxesList)) {
      if ($boxesId = $this->topic->getBoxesTopicID()) {
        if ($this->isPreview()) {
          include_once(PAPAYA_INCLUDE_PATH.'system/base_boxeslinks.php');
          $this->_boxesList = new base_boxeslinks($this->topic);
        } else {
          include_once(PAPAYA_INCLUDE_PATH.'system/papaya_publicboxeslinks.php');
          $this->_boxesList = new papaya_public_boxeslinks($this->topic);
        }
        $this->_boxesList->setPageId($boxesId);
  ...
  */
  function getBoxXML() {
    if (!$this->topic->topicExists($this->topicId)) {
      $this->topicId = $this->papaya()->options->get('PAPAYA_PAGE_DEFAULT', 0);
    }
    $this->topic->loadOutput(
      $this->topicId, $this->requestData['language'], $this->versionDateTime
    );
    ...
    if ($boxes->load($this->boxId, $this->topic->topic['TRANSLATION']['lng_id'])) {
      ...
      if (!simple_xmltree::isXML($result, $this)) {
  ...
  */
  function getBox($xml = FALSE) {
    ...
    if ((isset($this->output->viewMode) && !empty($this->output->viewMode['viewmode_ext'])) ||
      ...
      $boxes = new papaya_boxes;
      $loaded = $boxes->load(
        $this->boxId, $this->topic->languageIdentToId($this->requestData['language'])
      );
      if ($loaded) {
          ...
          if ($this->filter = $this->output->getFilter($viewId)) {
            if (!$useFilter) {
              ...
              $this->getError(
                ...
                'Output mode "'.
                  papaya_strings::escapeHTMLChars(basename($this->mode)).'"for page #'.
                  $this->topicId.' not configured: '.$this->filter->errorMessage,
                PAPAYA_PAGE_ERROR_OUTPUT_SETTINGS
              );
            }
          } else {
  ...
  */
  function executeController(&$controller) {
    $counter = 10;
    do {
      $controller = &$controller->execute($this);
    } while (is_object($controller) &&
             --$counter > 0);
    return is_bool($controller) ? $controller : FALSE;
  }
  ...
  */
  function loadMediaFileData($id, $version = NULL) {
    ...
    if ($fileData = $this->mediaDB->getFile($id, $version)) {
      return $fileData;
    }
    return FALSE;
  }
    ...
    if (!empty($this->requestData['media_id'])) {
      ...
      if (preg_match($mediaFilePattern, $this->requestData['media_id'], $matches)) {
        ...
        if ($file = $this->checkMediaPerm($matches[2])) {
          ...
          $fileName = PAPAYA_PATH_THUMBFILES.$path.$matches[1];
          if (isset($matches[9])) {
            $fileName .= $matches[9];
  ...
  */
  private function _storageSetPublic($storageGroup, $storageId, $mimeType) {
    $options = $this->papaya()->getObject('Options');
    $storage = PapayaMediaStorage::getService(
      $options->getOption('PAPAYA_MEDIA_STORAGE_SERVICE'),
      $options
    );
  ...
  * @access public
  * @return mixed string $mime['mimetype_text'] or NULL
  */
  function filenameToMimeType($fileName) {
    if (preg_match('/\.([\w\d]+)$/', $fileName, $regs)) {
      if (!(isset($this->mimeTypes) && is_object($this->mimeTypes))) {
        include_once(PAPAYA_INCLUDE_PATH.'system/base_mimetypes.php');
        $this->mimeTypes = new base_mimetypes();
      }
      if ($mime = $this->mimeTypes->loadMimeTypeByExt($regs[1])) {
        return $mime['mimetype_ext'];
      }
    }
  ...
  */
  function getXMLOutput() {
    $this->createTopic();
    if (!(PAPAYA_DBG_XML_OUTPUT && $this->validateEditorAccess())) {
      $this->getError(
        403,
        'Access forbidden',
        PAPAYA_PAGE_ERROR_ACCESS
      );
    } elseif (($this->boxId > 0) && $this->isPreview()) {
  ...
  */
  function getPageOutput() {
    if ($this->papaya()->options->getOption('PAPAYA_LOG_RUNTIME_REQUEST', FALSE)) {
      PapayaRequestLog::getInstance()->logTime('Page defined');
    }
    $this->createTopic();
    if (($this->boxId > 0) && $this->isPreview()) {
      print $this->getBox();
    } elseif ($this->topic->topicExists($this->topicId)) {
      $loaded = $this->topic->loadOutput(
        $this->topicId,
  ...
  */
  ...
  /**
  * This method returns the current microtime as a float number.
  *
  * @access private
  * @return float
  */
  ...
  * specific media file.
  *
  * @todo apply changes to mediadb permission model
  * @param integer $mid
  * @access public
  * @return array $file
  */
  function checkMediaPerm($mid) {
    ...
    if ($file) {
      ...
      if (!$this->isPreview() || !$this->papaya()->administrationUser->isValid) {
        ...
        $folderPermissions = $this->mediaDB->calculateFolderPermissions($file['folder_id']);
  ...
  */
  function logRequest($lngId = 0, $cachedPage = FALSE) {
    ...
    if ($this->papaya()->options->get('PAPAYA_PAGE_STATISTIC', FALSE)) {
      include_once(PAPAYA_INCLUDE_PATH.'system/base_statistic_logging.php');
      $statObj = new base_statistic_logging($this->msgs);
  ...
  */
  ...
  /**
  * This method initializes $this->startTime with the current microtime
  *
  * @access public
  */
  function startTimer() {
  ...
  /**
  * This method returns the time passed since startTimer() was called.
  *
  * @param integer $digits optional, default value 2
  * @access public
  * @return float time passed with $digits decimal places
  */
  function stopTimer($digits = 2) {
    $to = explode(' ', microtime());
    $from = explode(' ', $this->startTime);
    $to = $to[0] + (double)$to[1];
    $from = $from[1] + (double)$from[0];
    return number_format(($to - $from) * 1000, $digits);
  }

  /**
  * This method outputs a debug msg
  *
  * @param array $msg
  * @access public
  * @return integer
  */
  function showDebug($msg) {
    echo '</form>', '</table><br>';
    echo '<table style="position: relative; z-index: 10000;"'.
         ' border="1" width="800" cellpadding="2" cellspacing="0"'.
         ' align="center" bgcolor="#C0C0C0">';
    echo sprintf(
      '<tr><th>Debugausgabe (%s)</th><th align="right">'.
      $msg['time'].' ms</th></tr>',
      'Gesamtlaufzeit'
    );
    echo '</table><br/>';
  ...
  function getError($error, $errorString, $errorCode = NULL, $verbose = FALSE) {
    if (!$verbose) {
      ...
      if ((empty($_GET['redirect']) || $_GET['redirect'] != $errorStatus) &&
          $this->topic->topicId) {
        $targetUrl = $this->getAbsoluteURL(PAPAYA_PATH_WEB.$this->getBaseLink());
        $targetUrl .=
      ...
      if ($this->topic->topicId) {
              ...
              if ($this->filter = $this->output->getFilter($viewId)) {
                ...
                if ($this->filter->checkConfiguration()) {
                  ...
                  $this->getErrorHTML(
                    ...
                    'Output mode "'.
                      papaya_strings::escapeHTMLChars(basename($this->mode)).'"for page #'.
                      $this->topic->topicId.' not configured: '.$this->filter->errorMessage,
                    PAPAYA_PAGE_ERROR_OUTPUT_SETTINGS
                  );
                }
              } else {
        ...
        print $this->getErrorHTML($error, $errorString, $errorCode);
      }
    } else {
      print $this->getErrorHTML($error, $errorString, $errorCode);
    }
  ...
  */
  function getRedirect($status, $url) {
    ...
    switch ($status) {
    case 302:
      $this->topic->loadOutput((int)PAPAYA_PAGEID_STATUS_302, $lngIdent);
      break;
    case 301:
    default :
      $status = 301;
      $this->topic->loadOutput((int)PAPAYA_PAGEID_STATUS_301, $lngIdent);
      break;
    }
    $this->redirect = array(
      'status' => $status,
    ...
    if ($this->topic->topicId) {
      ...
      print $this->getRedirectHTML($status, $url);
    }
    exit();
  }
  ...
  *
  * @param string $headerString the string to be send as header
