...
* @subpackage Controls
* @version $Id: base_formdesigner.php 37900 2012-12-20 12:08:03Z smekal $
*/
...
* XML-Handling
*/
require_once(PAPAYA_INCLUDE_PATH.'system/base_formdsg_xml.php');
...
class base_formdesigner extends base_formdesigner_xml {
  ...
  * @access public
  */
  function initialize($paramName, $fieldXML, $lngId, $hiddenParams = NULL, $baseParams = NULL) {
  ...
  */
  function initFieldEditDialog($newField = FALSE) {
    ...
    if (!(isset($this->fieldEditDialog) && is_object($this->fieldEditDialog))) {
      ...
      $this->fieldEditDialog->msgs = &$this->msgs;
  ...
  function initGroupEditDialog($newField = FALSE) {
    ...
    if (!(isset($this->groupEditDialog) && is_object($this->groupEditDialog))) {
      ...
      $fields = array(
        ...
        'fielddsg_text' => array('Text', 'isSomeText', TRUE, 'simplerichtext', 6)
      );
      ...
      $this->groupEditDialog->msgs = &$this->msgs;
  ...
  *
  * @access public
  * @param array $params optional, default value empty array
  ...
  */
   ...
   * @todo country connector has to set it's paramName automatically
   */
  ...
  */
  function getMediaDirectoryCombo($name, $field, $data) {
    ...
    $this->folders = $mediaDB->getFolderComboArray($this->lngId);

    if ( isset($fileManager->folderLinks)) {
      $this->folderLinks = $fileManager->folderLinks;
    }
    ...
    foreach ($this->folders as $folderId => $folderName) {
