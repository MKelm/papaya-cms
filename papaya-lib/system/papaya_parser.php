...
* @package Papaya
* @subpackage Core
* @version $Id: papaya_parser.php 38082 2013-02-05 15:21:32Z weinert $
*/

/**
* Base object database access
*/
...
class papaya_parser extends base_db {
    ...
    if (preg_match_all($this->papayaParamsPattern, $tag, $matches, PREG_SET_ORDER)) {
      foreach ($matches as $match) {
        ...
        case 'addon':
          ...
          break;
        case 'href':
          if (checkit::isNum($paramValue, TRUE)) {
            $this->topics[(int)$paramValue] = FALSE;
          }
          break;
        }
    ...
    if (isset($this->addOns) && is_array($this->addOns)) {
      ...
      foreach ($addOns as $data) {
        $moduleObj = base_pluginloader::getPluginInstance(
          ...
          NULL,
          $data['module_class'],
          $data['module_path'].$data['module_file']
        );
      ...
      if ($res = $this->databaseQueryFmt($sql, $params)) {
        while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
          ...
          if (!isset($this->topicLinkTypes[$row['linktype_id']]) && $row['linktype_is_popup']) {
            ...
            $xmlTree = &simple_xmltree::createFromXML($row['linktype_popup_config'], $this);
            if (isset($xmlTree) && isset($xmlTree->documentElement) &&
              ...
              for ($idx = 0; $idx < $xmlTree->documentElement->childNodes->length; $idx++) {
                ...
                if (is_object($xmlNode) &&
                    $xmlNode->nodeType == XML_ELEMENT_NODE &&
  ...
  */
  function getStorageService() {
    if (!isset($this->storageService)) {
      $configuration = $this->papaya()->getObject('Options');
      $this->storageService = PapayaMediaStorage::getService(
        $configuration->getOption('PAPAYA_MEDIA_STORAGE_SERVICE', ''),
        $configuration
      );
    }
  ...
  function getImageConfInfos() {
    if (isset($this->dynImages) &&
      ...
      if ($res = $this->databaseQueryFmt($sql, $params)) {
        ...
        $this->papaya()->plugins->preload(array_keys($moduleGuids));
        foreach ($imageData as $identifier => $data) {
  ...
  function createTag(&$tag) {
    ...
    case 'addon':
      if (!empty($tag['params']['addon'])) {
        ...
        if (isset($this->addOns[$guid]) && is_object($this->addOns[$guid])) {
  ...
  */
  private function getDynamicFileExtension($format) {
    ...
    if (isset($extensions[$format])) {
      ...
      $systemFormat = $this->papaya()->options->getOption('PAPAYA_THUMBS_FILETYPE');
  ...
  */
  function createImageTag($params, $data) {
    ...
    if (isset($params['topic']) &&
    ...
    } elseif (isset($params['href']) && checkit::isNum($params['href'], TRUE) &&
      ...
      $hrefData = array(
        ...
        'title' => isset($params['title']) ? $params['title'] : ''
              ...
              checkit::isHTTPX($params['href'], TRUE) && $this->isSessionInUri()) {
      $hrefData = array(
  ...
  */
  function calcBorders($params, $data) {
  ...
  */
   ...
   * @return string
   */
  public static function getDataPopupAttribute($width, $height, $scrollbars, $resizable, $toolbar,
      $top = NULL, $left = NULL, $menubar = FALSE, $location = FALSE, $status = FALSE) {
