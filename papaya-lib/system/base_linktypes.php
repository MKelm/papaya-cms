...
* @package Papaya
* @subpackage Core
* @version $Id: base_linktypes.php 36224 2011-09-20 08:00:57Z weinert $
*/
...
class base_linktypes extends base_db {
  ...
  *
  * @param mixed $linkTypeIds single ID or array of linktype Ids or NULL
    ...
    foreach ($linkTypeIds as $linkTypeId) {
      if (isset($this->linkTypes[$linkTypeId])) {
        if (isset($this->linkTypes[$linkTypeId]['linktype_popup_config']) &&
          ...
          $popupConfigXML = $this->linkTypes[$linkTypeId]['linktype_popup_config'];
          $popupConfig = FALSE;
          simple_xmltree::unserializeArrayFromXML('data', $popupConfig, $popupConfigXML);
  ...
  */
  function getLinkType($linkTypeId, $forceLoading = FALSE) {
    return $this->getCompleteLinkTypes($linkTypeId, $forceLoading = FALSE);
  }
