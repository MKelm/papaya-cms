...
* @version $Id: base_mediadb.php 38455 2013-04-29 11:43:24Z weinert $
*/

/**
* load basic db functions
*/
...
class base_mediadb extends base_db {
  ...
  */
  ...
  /**
  * @var $fallbackMimeType mimetype to choose if no other can be identified
  */
  ...
  */
  ...
  /**
  * php 4 constructor calls __construct
  */
  function base_mediadb() {
    base_mediadb::__construct();
  }
  ...
  public function getItem($mediaId, $versionId = NULL) {
    $item = new PapayaMediaDatabaseItem(
      PapayaMediaStorage::getService(
        $this->papaya()->getObject('Options')->getOption('PAPAYA_MEDIA_STORAGE_SERVICE'),
        $this->papaya()->getObject('Options')
      )
    );
  ...
  * @return array
  */
  function getFilesTranslated($lngId, $folderId,
  ...
  */
  function findFiles($searchParams, $limit = NULL, $offset = 0, $sort = 'name', $order = 'ASC') {
    ...
    if (!empty($searchParams['q'])) {
      PapayaAutoloader::load(
        'searchStringParser', PAPAYA_INCLUDE_PATH.'system/base_searchstringparser.php'
      );
    ...
    if (!empty($searchParams['smaller'])) {
      $conditions[] = sprintf(
          " (f.file_size > %s) ",
          $searchParams['smaller'] * 1024
      );
    }
    if (!empty($searchParams['bigger'])) {
      $conditions[] = sprintf(
        " (f.file_size < %s) ",
          $searchParams['bigger'] * 1024
      );
    }
  ...
  *
  * @todo conceive and implement metadata structure and content
  * @todo implement plugin system for arbitrary filetypes metadata
  *
  * @param string $fileLocation location of file on local disk
  * @param string $originalFileName original name of the file (no path)
  * @param string $fileType optional file type sent at upload, only to check it
  ...
  */
  function getFileProperties($fileLocation, $originalFileName, $fileType = '') {
    ...
    $result['file_source'] = '';
    $result['file_source_url'] = '';
    $result['file_keywords'] = '';
    $mimeTypeImage = array();
  ...
  */
  function getFlvViewer($fileId, $width, $height, $noFlash = '',
                        $bgColor = '#000000', $configuration = array(),
                        $scale = 'noscale', $svalign = '', $shalign = '',
                        $quality = 'high') {
  ...
  */
  function callbackFolders($name, $field, $data) {
    if (defined('PAPAYA_ADMIN_PAGE') && PAPAYA_ADMIN_PAGE) {
      ...
      $folders = $mediaDB->getFolderComboArray($lngSelect->currentLanguageId);
