...
*   papaya_taglinks::getInstance(&$this);
...
* @version $Id: papaya_taglinks.php 38396 2013-04-15 16:35:51Z weinert $
*/
...
class papaya_taglinks extends base_tags {
  ...
  */
  var $tagType = 'topic';
  /**
  * @var integer $linkId id of $tagType item to be tagged
  */
  var $linkId = NULL;
  ...
  */
  ...
  /**
  * papaya 4 constructor
  */
  function papaya_taglinks(&$parentObj, $paramName = 'tg') {
    $this->__construct($parentObj, $paramName);
  }

  /**
  * get an instance of base_taglinks
  *
  * @param object $parentObj parent object, must hold msgs, authUser, images
  * @return object $tagLinks instance of base_taglinks
  */
  function &getInstance(&$parentObj, $paramName = NULL) {
    static $instance;
    if (isset($instance) &&
        is_object($instance) &&
        is_a($instance, 'papaya_taglinks')) {
      return $instance;
    } elseif ($parentObj->authUser->hasPerm(PapayaAdministrationPermissions::TAG_LINK)
        && isset($parentObj) && is_object($parentObj)
        && isset($parentObj->msgs) && isset($parentObj->authUser)
        && isset($parentObj->images)
    ) {
      if ($paramName == NULL) {
        $paramName = $parentObj->paramName;
      }
      $instance = new papaya_taglinks($parentObj, $paramName);
      $instance->msgs = &$parentObj->msgs;
      $instance->authUser = &$parentObj->authUser;
      $instance->images = &$parentObj->images;
      return $instance;
    }
  }
  ...
  * @param string $type whether you can select a 'single' or 'multiple' tags
  ...
  */
  function getTagLinkingXML() {
    ...
    if ($this->authUser->hasPerm(PapayaAdministrationPermissions::TAG_LINK) &&
  ...
  */
  function getLinkedTagsListXML() {
    ...
    $linkCategories = $this->getCategories(
      $linkCatIds, $this->parentObj->lngSelect->currentLanguageId
    );
    $allParents = array();
  ...
  */
  function getTagTitle(&$alternativeTags, $currentTag, $otherTags) {
    ...
    } elseif (PAPAYA_CONTENT_LANGUAGE != $this->lngSelect->currentLanguageId) {
      if (!(
        ...
        $alternativeTags = $this->getTags(
          array_keys($otherTags), PAPAYA_CONTENT_LANGUAGE
        );
      }
  ...
  function getXMLCategoryEntry($id, $indent, $mode = TRUE) {
    ...
    if (isset($this->categories[$id]) && is_array($this->categories[$id])) {
      ...
      $result .= sprintf(
        ...
        papaya_strings::escapeHTMLChars($this->getLink(array('cat_id' => (int)$id))),
  ...
  */
  function loadCategories() {
    ...
    $lngCondition = $this->databaseGetSQLCondition(
      'ct.lng_id', $this->parentObj->lngSelect->currentLanguageId
    );
    $parentCondition = $this->databaseGetSQLCondition('c.parent_id', $parentIds);
    $availableCategories = $this->getAvailableCategories($this->authUser, array('user_use_tags'));
  ...
  */
  function loadAlternativeCategoryNames($categoryIds) {
    if (PAPAYA_CONTENT_LANGUAGE != $this->lngSelect->currentLanguageId &&
      ...
      $params = array($this->tableTagCategoryTrans, PAPAYA_CONTENT_LANGUAGE);
      if ($res = $this->databaseQueryFmt($sql, $params)) {
        while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
  ...
  function getTagsPagingBar($absCount, $cmd = NULL) {
    ...
    if ($absCount > 10) {
      ...
      $result .= papaya_paging_buttons::getButtons(
        $this, array('cmd' => $cmd), $this->pagingSteps, $steps,
        'limit', 'right');
      $result .= '</buttons>'.LF;
    }
    return $result;
  ...
  */
  function addTag($tagName) {
    $data = array(
      ...
      'default_lng_id' => $this->lngSelect->currentLanguageId,
      'creator_type' => 'admin',
      'creator_id' => $this->authUser->userId,
      'creation_time' => time(),
    );
  ...
  */
  function getSearchResult() {
    ...
    $categories = $this->getCategories($catIds, $this->lngSelect->currentLanguageId);
    ...
    if (isset($tags) && is_array($tags) && count($tags) > 0) {
      ...
      foreach ($tags as $tag) {
        if (isset($this->linkedTags[$tag['tag_id']])) {
          ...
          $tagIcon = $this->images['actions-list-remove'];
        } else {
          $linkParams = array(
            'cmd' => 'link_tag',
            'tag_id' => $tag['tag_id'],
          );
          $tagIcon = $this->images['actions-list-add'];
        }
        ...
        $result .= sprintf(
          '<listitem image="./pics/language/%s" title="%s (%d)">'.LF,
          papaya_strings::escapeHTMLChars(
            $this->lngSelect->languages[$tag['lng_id']]['lng_glyph']
          ),
  ...
  public function callbackFormatPriority($context, $priority) {
    return $priority.'%';
  }
