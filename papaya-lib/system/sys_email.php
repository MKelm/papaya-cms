...
* @subpackage Email
* @version $Id: sys_email.php 37248 2012-07-18 11:48:02Z weinert $
*/
...
class email extends base_object {
  ...
  * @return boolean
  */
  function addAttachmentData($fileTitle, $data,
  ...
  */
  function embedImage($fileSrc) {
      ...
      if (preg_match($pImage, $fileSrc, $regs)) {
        ...
        if (isset($regs[6]) &&
          ...
          $fileData = $this->imgGenerator->generateImage(
            FALSE, $params['img'], PAPAYA_THUMBS_FILETYPE
          );
          ...
          $this->addImageData(
            ...
            $fileData,
            $cId,
            image_type_to_mime_type(PAPAYA_THUMBS_FILETYPE)
          );
          return 'cid:'.$cId;
        }
      }
    ...
    case 'media' :
      ...
      case 'thumb' :
      case 'thumbnail' :
        if (file_exists(PAPAYA_PATH_THUMBFILES.$path.$localFile)) {
          $fileName = PAPAYA_PATH_THUMBFILES.$path.$localFile;
          break;
        }
      case 'media' :
        ...
        if (file_exists(PAPAYA_PATH_MEDIAFILES.$path.$localFile)) {
          $fileName = PAPAYA_PATH_MEDIAFILES.$path.$localFile;
        } else {
    ...
    if (isset($fileName) && is_file($fileName) && is_readable($fileName)) {
      ...
      $mimeType = image_type_to_mime_type(PAPAYA_THUMBS_FILETYPE);
      if (isset($mimeType)) {
        $cId = $this->generateContentId();
  ...
  */
  function setReturnPath($email) {
    // If we got anything else than an email address, issue an error
    include_once(PAPAYA_INCLUDE_PATH.'system/sys_checkit.php');
    if (!checkit::isEMail($email)) {
  ...
  */
  function getMimeType($hasText = FALSE, $hasHTML = FALSE,
                       $hasImages = FALSE, $hasAttachments = FALSE) {
    if ($hasAttachments) {
  ...
  */
  function getMessagePart($msgPartType, $boundary) {
    ...
    case 'multipart/related':
      ...
      $boundarySubPart = $this->createBoundary();
      if ($msgPartType['multipart']) {
        $encoding = $this->encoding;
      } else {
        $encoding = ($msgSubPartType['mimetype'] == 'text/html')
          ? $this->encodingHTML : $this->encodingText;
      }
      $result .= $this->getBoundaryStart(
        $boundary, '', $msgSubPartType['mimetype'], '', $boundarySubPart
      );
  ...
  */
  function send($to = NULL, $subject = NULL, $body = NULL) {
    ...
    if (0 === strpos(PHP_OS, 'WIN')) {
      ...
      if (isset($this->returnPath) && checkit::isEMail($this->returnPath) &&
          !$safeMode) {
        $result = @mail(
