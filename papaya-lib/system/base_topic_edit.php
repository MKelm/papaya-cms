...
* @package Papaya
* @subpackage Core
* @version $Id: base_topic_edit.php 38384 2013-04-10 14:50:30Z weinert $
*/

/**
* Basic class database access
*/
require_once(PAPAYA_INCLUDE_PATH.'system/base_topic.php');
...
class base_topic_edit extends base_topic {
  /**
  * language selector
  * @var object base_language_select $lngSelect
  */
  var $lngSelect = NULL;
  ...
  */
  function execute() {
    ...
    $this->switchRichtext = &base_switch_richtext::getInstance();

    $this->load($this->params['page_id'], $this->lngSelect->currentLanguageId);
    $this->currentLanguage = $this->lngSelect->currentLanguage;

    $this->layout->setParam('COLUMNWIDTH_LEFT', '250px');

    include_once(PAPAYA_INCLUDE_PATH.'system/papaya_locking.php');
    ...
    $locked = $this->lockingObj->setLock($this->authUser->userId, 1, $this->lockingIdent);
    if (!$locked) {
    ...
    case 'move_position' :
      if ($this->hasPermUser(PERM_WRITE, $this->authUser) &&
          $this->editable($this->authUser)) {
        if (preg_match('(^\d+$)D', $this->topic['topic_weight']) &&
          ...
          if ($this->papaya()->options->getOption('PAPAYA_LOG_EVENT_PAGE_MOVED', TRUE)) {
    ...
    case 'add_translation':
      if ($this->hasPermUser(PERM_WRITE, $this->authUser) &&
        ...
        if (isset($this->params['lng_id']) && $this->params['lng_id'] > 0 &&
            $this->lngSelect->currentLanguageId == $this->params['lng_id'] &&
            isset($this->lngSelect->languages[(int)$this->params['lng_id']])) {
          if ($this->createTranslation(
              ...
              new PapayaMessageLog(
                ...
                new PapayaUiString(
                  ...
                  array(
                    ...
                    $this->lngSelect->languages[(int)$this->params['lng_id']]['lng_short'],
                    $this->topicId
                  )
                )
              )
            ...
            $this->sychronizations()->synchronizeAction(
              ...
              $this->topicId,
              array($this->lngSelect->currentLanguageId)
            );
            $this->loadTranslatedData(
              $this->topicId, $this->lngSelect->currentLanguageId
            );
            $this->currentLanguage = $this->lngSelect->currentLanguage;
          }
        }
    ...
    case 'add_topic':
      if ((
           $this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_CREATE) &&
           $this->hasPermUser(PERM_CREATE, $this->authUser)
          ) &&
          (
           $this->authUser->startNode == 0 ||
           $this->hasParent($this->authUser->startNode)
          ) &&
          (
           $this->getLevel($this->authUser->startNode) < $this->authUser->subLevel ||
           $this->authUser->subLevel == 0
          )
         ) {
    ...
    case 'import' :
      if ($this->hasImportView()) {
        ...
        if (isset($this->params['import_confirm']) && $this->params['import_confirm']) {
          include_once(PAPAYA_INCLUDE_PATH.'system/base_import.php');
          $importer = new base_import();
          $importer->msgs = &$this->msgs;
          $importer->images = &$this->images;
          ...
          if ($importData) {
            ...
            $plugin = &base_pluginloader::getPluginInstance(
              $moduleData['module_guid'],
              $this,
              NULL,
              $moduleData['module_class'],
              $moduleData['module_path'].$moduleData['module_file']
            );
            if (isset($plugin) && is_object($plugin)) {
              $plugin->data = $importData;
            ...
            if ($this->deleteTrans($lng)) {
              ...
              $this->logMsg(
                ...
                sprintf(
                  ...
                  $this->lngSelect->languages[(int)$lng]['lng_short'],
                  $this->topic['TRANSLATION']['topic_title'],
                  (int)$this->topicId
                )
              );
    ...
    if (!$this->topicId && $this->hasParent($this->authUser->user['start_node']) ||
         (
            $this->topic['is_deleted'] &&
            !$this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_TRASH_MANAGE)
         )
       ) {
      if ($this->params['page_id'] != $this->authUser->user['start_node'] &&
        ...
        $toUrl = $protocol."://".$_SERVER['HTTP_HOST'].$this->getBasePath().
          $this->getLink(
            array(
              'redirected' => 1,
              'page_id' => $this->authUser->user['start_node']
            )
          );
      ...
      $this->menubar->addButton(
        ...
        'Create a new page'
                ...
                $this->hasPermUser(PERM_WRITE, $this->authUser) &&
                $this->editable($this->authUser)
              )) {
      ...
      case 2:  //boxes
        ...
        if ($dependencyBlocker->isSynchronized(PapayaContentPageDependency::SYNC_BOXES)) {
          $this->layout->add($dependencyBlocker->getXml());
        } elseif ($this->authUser->hasPerm(PapayaAdministrationPermissions::BOX_LINK)) {
          include_once(PAPAYA_INCLUDE_PATH.'system/papaya_boxeslinks.php');
          $boxLinks = new papaya_boxeslinks($this);
          $boxLinks->msgs = &$this->msgs;
          $boxLinks->authUser = &$this->authUser;
          $boxLinks->images = &$this->images;
          $boxLinks->initialize();
          if (isset($this->params['chgusep']) && (bool)$this->params['chgusep']) {
            if ($this->saveBoxUseParent()) {
              $this->load($this->topicId, $this->lngSelect->currentLanguageId);
              $this->sychronizations()->synchronizeAction(
          ...
          $this->layout->addRight($this->getEditUseParentForm($this->getBoxParent()));
          if (!$this->topic['box_useparent']) {
            $boxLinks->topicId = $this->topicId;
            $boxLinks->loadList();
            ...
            $this->layout->add(
              $boxLinks->getList($this->images, TRUE, $this->_gt('Linked boxes'))
            );
            $this->layout->add($boxLinks->getBoxList($this->images));
          } elseif ($boxLinks->topicId = $this->getBoxParent()) {
            $boxLinks->loadList();
            $boxLinks->loadBoxList();
            $boxLinks->loadBoxGroupList();
            $this->layout->add(
              $boxLinks->getList($this->images, FALSE, $this->_gt('Inherited boxes'))
            );
          }
        } else {
          $this->addMsg(
      ...
      case 4: // surfer permissions
        ...
        if ($dependencyBlocker->isSynchronized(PapayaContentPageDependency::SYNC_ACCESS)) {
          $this->layout->add($dependencyBlocker->getXml());
        } elseif ($this->authUser->hasPerm(2, '88236ef1454768e23787103f46d711c2')) {
          include_once(PAPAYA_INCLUDE_PATH.'system/base_surferlinks.php');
          $sfl = new base_surferlinks($this->topicId);
          $sfl->msgs = &$this->msgs;
          $sfl->images = &$this->images;
          if ($sfl->execute()) {
            $this->sychronizations()->synchronizeAction(
          ...
          $this->layout->add($sfl->get($this->lngSelect->currentLanguageId));
      ...
      case 7: //versions
        ...
        $this->layout->add($this->publishExecute($this->msgs, $this->authUser));
        $this->load($this->topicId, $this->lngSelect->currentLanguageId);
        $this->loadTranslationsInfo();
        $this->layout->add($this->getPublicData());
        // backup versions
        if ($this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_VERSION_MANAGE)) {
          $this->loadVersionsList();
          if (isset($this->params['version_id']) &&
              $this->loadVersion($this->params['version_id'])) {
            $this->layout->add($this->versionExecute($this->msgs, $this->authUser));
      ...
      case 8: //editor permissions
        if ($this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_PERMISSION_MANAGE)) {
      ...
      case 10: // tags
        ...
        if ($dependencyBlocker->isSynchronized(PapayaContentPageDependency::SYNC_TAGS)) {
          $this->layout->add($dependencyBlocker->getXml());
        } elseif ($this->authUser->hasPerm(PapayaAdministrationPermissions::TAG_MANAGE)) {
          include_once(PAPAYA_INCLUDE_PATH.'system/papaya_taglinks.php');
          $tags = papaya_taglinks::getInstance($this);
          if ($tags->prepare('topic', $this->topicId)) {
            if ($tags->execute()) {
              $this->sychronizations()->synchronizeAction(
      ...
      case 11 : // dependencies
        if ($this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_DEPENDENCY_MANAGE)) {
    ...
    $topicTree->images = &$this->images;
    $topicTree->msgs = &$this->msgs;
    $topicTree->layout = &$this->layout;
    $topicTree->authUser = &$this->authUser;
  ...
  public function embedPreview() {
    ...
    $this->menubar->addButton(
      'Full window',
      $link,
      $this->images['actions-view-fullscreen'],
      'Full window preview'
    );
  }
    ...
    case 'del_topic':
      if (isset($this->params['del_topic_confirm']) && isset($this->topic)) {
        ...
        if ($this->topic['is_deleted'] &&
            $this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_TRASH_MANAGE) &&
              $this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_DELETE)) {
          if ($this->destroy()) {
            $this->addMsg(
              MSG_INFO,
              $this->_gtf('Page #%d deleted.', $this->topicId),
              TRUE,
              PAPAYA_LOGTYPE_PAGES
            );
            unset($topic);
          ...
          $this->addMsg(
            MSG_INFO,
            $this->_gtf('Page #%d moved to trash.', $this->topicId),
            TRUE,
            PAPAYA_LOGTYPE_PAGES
          );
          unset($topic);
  ...
  */
  function getToolbar() {
    ...
    if ($this->hasPermUser(PERM_WRITE, $this->authUser) && $this->editable($this->authUser)) {
      $toolbar->addButton(
        'Properties',
        $this->getLink(
      ...
      if ($this->authUser->hasPerm(PapayaAdministrationPermissions::BOX_LINK)) {
        $toolbar->addButton(
          'Boxes',
          $this->getLink(
      ...
      if ($this->authUser->hasPerm(PapayaAdministrationPermissions::TAG_MANAGE)) {
        $toolbar->addButton(
          'Tags',
          $this->getLink(
      ...
      if ($this->authUser->hasPerm(2, '88236ef1454768e23787103f46d711c2')) {
        $toolbar->addButton(
          'Access',
          $this->getLink(
    ...
    } elseif ($this->hasPermUser(PERM_READ, $this->authUser)) {
      $toolbar->addButton(
        'Properties',
        $this->getLink(
          array('cmd' => 'chg_mode', 'mode' => 0, 'page_id' => $this->topicId)
  ...
  */
  function setMenuBar($editable = TRUE) {
    $this->menubar->clear();
    if ($this->hasPermUser(PERM_CREATE, $this->authUser, $this->topic['prev'])) {
      $this->menubar->addButton(
        'Add page',
        $this->getLink(
          array(
    ...
    if ($this->hasPermUser(PERM_CREATE, $this->authUser)) {
      $this->menubar->addButton(
        'Add subpage',
        $this->getLink(
          array(
    ...
    if ($editable) {
      ...
      if ($this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_DELETE)) {
        $this->menubar->addButton(
          'Delete page',
          $this->getLink(
            array(
  ...
  function saveProperties() {
    if ($this->topicId == $this->dialogProperties->params['page_id']) {
      ...
      if (isset($this->topic['TRANSLATION'])) {
        ...
        if ($this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_METADATA_EDIT) &&
  ...
  function create() {
    if ($this->topicId > 0 && isset($this->topic) && is_array($this->topic)) {
      ...
      $data = array(
        ...
        'topic_mainlanguage' => 0,
        'author_id' => $this->authUser->userId,
        'author_group' => $this->authUser->user['group_id'],
        'author_perm' => '777',
        'linktype_id' => 1,
        'topic_protocol' => 0,
        'prev' => 0,
  ...
  */
  function createVersion($publishedLngs = NULL) {
    ...
    if ($newVersionId = $this->databaseInsertRecord(
        ...
        if ($res = $this->databaseQueryFmt($sql, array($this->tableTopicsPublic, $this->topicId))) {
          ...
          if ($result) {
            ...
            if (count($languages) > 0) {
              ...
              if (FALSE !== $this->databaseDeleteRecord(
                    $this->tableTopicsPublicTrans, $params)) {
                ...
                $now = time();
                $params = array($this->tableTopicsPublicTrans, $now, $now,
                  $this->tableTopicsTrans, $this->topicId);
                if (FALSE !== $this->databaseQueryFmtWrite($sql, $params)) {
                  $this->deleteCache();
                  return TRUE;
  ...
  */
  function deletePublicTopic() {
    if ($this->databaseDeleteRecord(
          $this->tableTopicsPublicTrans, 'topic_id', $this->topicId) !== FALSE) {
      if ($this->databaseDeleteRecord(
            $this->tableTopicsPublic, 'topic_id', $this->topicId) !== FALSE) {
        $this->deleteCache();
        return TRUE;
      }
    }
  ...
  */
  function deletePublicTopicTrans($lngId = NULL) {
    ...
    $sql = "SELECT COUNT(*)
             ...
             WHERE topic_id = %d";
    if (FALSE !== $this->databaseDeleteRecord(
          $this->tableTopicsPublicTrans, $condition)) {
      if ($res = $this->databaseQueryFmt(
            $sql, array($this->tableTopicsPublicTrans, $this->topicId))) {
        $row = $res->fetchRow();
        if ($row[0] == 0) {
  ...
  * @param mixed integer $topicId optional, default value NULL
  ...
  */
  function loadVersion($versionId, $topicId = NULL, $versionTime = 0) {
    ...
    if (isset($topicId) && $versionTime > 0) {
      $sql = "SELECT t.version_id, t.version_time, t.version_message, t.version_author_id,
                     t.topic_id, t.topic_mainlanguage, t.topic_weight,
                     t.topic_modified,
                     t.linktype_id, t.topic_protocol,
                     t.meta_useparent, t.box_useparent,
                     u.user_id, u.givenname, u.surname
                FROM %s t
                LEFT OUTER JOIN %s u ON t.version_author_id = u.user_id
               WHERE t.topic_id = '%d' AND t.version_time <= %d
               ORDER BY t.version_time DESC, t.version_id DESC";
      $params = array($this->tableTopicsVersions, $this->tableAuthUser,
        $topicId, $versionTime);
    } else {
      $sql = "SELECT t.version_id, t.version_time, t.version_message, t.version_author_id,
                     t.topic_id, t.topic_mainlanguage, t.topic_weight,
  ...
  */
  function initializePropertiesDialog() {
    if (!(isset($this->dialogProperties) && is_object($this->dialogProperties))) {
      ...
      if ($this->authUser->hasPerm(PapayaAdministrationPermissions::PAGE_CACHE_CONFIGURE)) {
  ...
  */
  function getLinkTypes() {
    $result = array();
    include_once(PAPAYA_INCLUDE_PATH.'system/base_linktypes.php');
    ...
    if (is_array($linkTypes) && count($linkTypes) > 0) {
      ...
      $result = array(1 => $this->_gt('visible'), 0 => $this->_gt('hidden'));
    }
    return $result;
  }
  ...
  */
  function getEditPermElement($caption, $perm) {
    ...
    if ($this->authUser->userId == $this->topic['author_id']) {
      $result .= sprintf(
        '<glyph src="%s"/>',
      ...
      $result .= sprintf(
        ...
        papaya_strings::escapeHTMLChars(
          $this->hasPerm($perm, PERM_OWNER)
            ? $this->images['status-node-checked']
            : $this->images['status-node-empty']
        )
      );
    }
    $result .= '</subitem>';
    $result .= '<subitem align="center">';
    if ($this->authUser->inGroup($this->topic['author_group']) &&
        $this->authUser->userId != $this->topic['author_id'] &&
  ...
  */
  function getEditUser($editUser) {
    ...
    if (isset($editUser->users) && is_array($editUser->users) &&
      ...
      foreach ($editUser->users as $username => $aUser) {
        ...
        if ($dependency->isOrigin($this->topicId)) {
          ...
          if (($dependency->synchronization & PapayaContentPageDependency::SYNC_VIEW) xor
            ...
            $originTranslation->load(
              array($dependency->originId, $this->topic['TRANSLATION']['lng_id'])
            );
  ...
  */
  function hasImportView() {
    if (isset($this->topic['TRANSLATION']['view_id']) &&
      ...
      include_once(PAPAYA_INCLUDE_PATH.'system/papaya_import.php');
  ...
  */
  function setUser($uid, $gid) {
    $sql = "SELECT user_id
             ...
             WHERE user_id = '%s'";
    if ($res = $this->databaseQueryFmt(
          $sql, array($this->tableAuthUser, $uid))) {
      if ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        $cUid = $row['user_id'];
        $sql = "SELECT group_id
                  FROM %s
        ...
        if ($gid == -1) {
          $cGid = -1;
        } elseif ($res = $this->databaseQueryFmt(
                    $sql, array($this->tableAuthGroups, $gid))) {
          if ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
            $cGid = $row['group_id'];
          }
        }
      ...
      case 'chmod':
        if ($this->params['val'] > 0 && isset($this->params['perm'])) {
          ...
          if ($this->hasPermUser($perm, $user) ==
                $this->hasPermUser($perm, $this->authUser, NULL, $permVal)) {
  ...
  * @return boolean
  */
  function saveBoxUseParent() {
    $deleted = $this->databaseDeleteRecord($this->tableBoxesLinks, 'topic_id', $this->topicId);
    if (FALSE !== $deleted) {
      if ($this->topic['box_useparent']) {
  ...
  *
  * @param integer $gotoId
  * @access public
  * @return string
  */
  function getEditUseParentForm($gotoId) {
    $result = '';
    if ($this->topic['box_useparent']) {
      $str = 'Define own boxes for this page?';
      $linkString = 'Yes, activate!';
    } else {
      $str = 'This page has own boxes.';
      $linkString = 'Disable';
    }
    ...
    $dialog = new base_msgdialog(
      $this, $this->paramName, $hidden, $this->_gt($str), 'question'
    );
    $dialog->msgs = &$this->msgs;
    $dialog->buttonTitle = $linkString;
    $result .= $dialog->getMsgDialog();

    if ($gotoId > 0 && $this->topic['box_useparent']) {
      $hidden = array(
        'cmd' => 'show',
        'page_id' => (int)$gotoId,
      );
      $msg = $this->_gt('Inherit boxes from a parent page.');
      $dialog = new base_msgdialog(
        $this, $this->paramName, $hidden, $msg, 'information'
      );
      $dialog->msgs = &$this->msgs;
      ...
      if ($res = $this->databaseQueryFmt($sql, array($this->tableSurferPerm))) {
        while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
          ...
          if ($this->topic['surfer_useparent'] == 1) {
            $image = $this->images['items-permission'];
          } elseif (isset($ownPermIds) &&
                    is_array($ownPermIds) &&
                    in_array($row['surferperm_id'], $ownPermIds)) {
            $image = $this->images['items-permission'];
          } else {
            $image = $this->images['status-permission-inherited'];
          }
          $result .= sprintf(
      ...
      case 'restore_version' :
        if (isset($this->params['restoreconfirm']) &&
          ...
          if (isset($this->params['restore_languages']) &&
            ...
            foreach ($this->params['restore_languages'] as $i => $lngId) {
  ...
  function callbackRestoreLanguages($name, $element, $data) {
    ...
    if (isset($this->lngSelect->languages) &&
      ...
      if ($res = $this->databaseQueryFmt($sql, $params)) {
        while ($row = $res->fetchRow()) {
          $lngId = $row[0];
          if (isset($this->lngSelect->languages[$lngId])) {
      ...
      foreach ($this->lngSelect->languages as $lngId => $lng) {
        ...
        if (isset($this->topic['TRANSLATIONINFOS']) &&
          ...
          if (isset($this->topic['TRANSLATIONINFOS'][$lngId]['topic_trans_published'])) {
            ...
            $result .= sprintf(
              '<subitem align="center"><glyph src="%s" hint="%s"/></subitem>',
              papaya_strings::escapeHTMLChars($this->images['status-page-created']),
      ...
      case 'handoff':
        ...
        if (isset($this->params['handoff_confirm']) &&
          ...
          $this->todo->params = array(
            ...
            'status' => 0,
            'date_to' => 0,
            'user_id_from' => $this->authUser->userId,
  ...
  */
  function initializePublishDialog() {
    if (!(isset($this->dialogPublish) && is_object($this->dialogPublish)) &&
      ...
      $dependenciesCount = $counter->getDependencies();
  ...
  */
  function initializeHandoffDialog() {
    if (!(isset($this->dialogHandoff) && is_object($this->dialogHandoff)) &&
      ...
      $this->todo = new papaya_todo;

      $fields = array(
        'user_id_to' => array('User', 'isSometext', TRUE, 'combo',
          $this->todo->getUserList(26), '', ''),
    ...
    if (isset($this->lngSelect->languages) && is_array($this->lngSelect->languages)) {
      foreach ($this->lngSelect->languages as $lngId=>$lng) {
        if (isset($this->topic['TRANSLATIONINFOS']) &&
          ...
          } elseif ((!is_array($data)) && $this->lngSelect->currentLanguageId == $lngId) {
            $selected = ' checked="checked"';
          } else {
            $selected = '';
          }
  ...
  * @return string
  */
  function callbackDeleteLanguages($name, $element, $data) {
    $result = '';
    if (isset($this->lngSelect->languages) &&
        is_array($this->lngSelect->languages)) {
      ...
      if (isset($languages) && is_array($languages)) {
        foreach ($languages as $lngId => $lng) {
          ...
          $result .= papaya_strings::escapeHTMLChars($languages[$lngId]['lng_title']).' ('.
            papaya_strings::escapeHTMLChars($this->lngSelect->languages[$lngId]['lng_short']).')';
        }
      }
    }
  ...
  function initializeAddTranslationDialog() {
    if (!(isset($this->dialogAddTranslation) && is_object($this->dialogAddTranslation))) {
      ...
      if (isset($this->topic['TRANSLATIONINFOS']) &&
          is_array($this->topic['TRANSLATIONINFOS'])) {
        foreach ($this->topic['TRANSLATIONINFOS'] as $translation) {
          if (isset($this->lngSelect->languages[$translation['lng_id']])) {
  ...
  */
  function getContentLanguageCombo($name, $element, $data) {
    ...
    if (!empty($this->lngSelect->languages[$lngId])) {
      $currentMainLanguage = ': '.$this->lngSelect->languages[$lngId]['lng_title'];
      $currentMainLanguage .= ' ('.$this->lngSelect->languages[$lngId]['lng_short'].')';
    } else {
      $currentMainLanguage = '';
    }
    return $this->lngSelect->getContentLanguageCombo(
      $this->paramName,
      $name,
      $element,
      $data,
