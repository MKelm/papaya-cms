...
* @package Papaya
* @subpackage Core
* @version $Id: papaya_datafilter_list.php 36224 2011-09-20 08:00:57Z weinert $
*/

/**
* data filter plugins handler/collection
*/
require_once(PAPAYA_INCLUDE_PATH.'system/base_datafilter_list.php');
...
class papaya_datafilter_list extends base_datafilter_list {
  ...
  function executeFilterLinkEdit($viewId, $filterId) {
    ...
    if ($this->loadFilterLink($viewId, $filterId)) {
      ...
      $moduleObj = &base_pluginloader::getPluginInstance(
        ...
        $parent,
        NULL,
        $this->filterLink['module_class'],
        $this->filterLink['module_path'].$this->filterLink['module_file']
      );
      if (isset($moduleObj) && is_object($moduleObj)) {
        $moduleObj->images = &$this->images;
        $moduleObj->paramName = $this->paramName;
        $moduleObj->setData($this->filterLink['datafilter_data']);
        $hidden = array(
  ...
  */
  function initializeFilterDialog() {
    if (!(isset($this->filterDialog) && is_object($this->filterDialog))) {
      ...
      $this->filterDialog->msgs = &$this->msgs;
  ...
  * Filter module used?
  *
  * @param string $modulGuid filter module guid
  * @access public
  * @return boolean used or not
  */
