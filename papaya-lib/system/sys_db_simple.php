...
* @package Papaya
* @subpackage Database
* @version $Id: sys_db_simple.php 38361 2013-04-04 12:09:41Z hapke $
*/
...
class db_simple extends base_object {
  ...
  * connect to database
  *
  * @param base_db $object calling object
  * @param boolean $readOnly use read connection
  * @return object dbcon_base
  */
  function connect($object, $readOnly = TRUE) {
    ...
    if (!(
          ...
          $this->databaseObjects[$mode]->connect(NULL, $readOnly)
        )) {
    ...
    if (!(isset($this->databaseObjects[$mode]) && is_object($this->databaseObjects[$mode]))) {
      ...
      return FALSE;
    } elseif ($this->databaseObjects[$mode]->extensionFound()) {
      return $this->databaseObjects[$mode]->connect(NULL, $readOnly);
    } else {
      unset($this->databaseObjects[$mode]);
      return FALSE;
    }
  ...
  */
  function close() {
    if ($this->papaya()->options->getOption('PAPAYA_LOG_RUNTIME_DATABASE', FALSE)) {
    ...
    if ($this->databaseObjects['read'] && is_object($this->databaseObjects['read'])) {
      $this->databaseObjects['read']->close();
    }
    if ($this->databaseObjects['write'] && is_object($this->databaseObjects['write'])) {
      $this->databaseObjects['write']->close();
    }
  }
  ...
  */
  function createConnection($object, $readOnly = TRUE) {
    ...
    if (isset($this->databaseObjects[$mode]) &&
        is_object($this->databaseObjects[$mode])) {
      if ($this->databaseObjects[$mode]->extensionFound()) {
        $this->databaseObjects[$mode]->connect();
        return TRUE;
      }
    } else {
      $file = realpath(
        PAPAYA_INCLUDE_PATH.'system/db/'.strtolower($this->databaseConfiguration[$mode]->api).'.php'
      );
      if (file_exists($file)) {
        ...
        if ($found) {
          $class = 'dbcon_'.$this->databaseConfiguration[$mode]->api;
          $this->databaseObjects[$mode] = new $class($this->databaseConfiguration[$mode]);
          $this->databaseObjects[$mode]->extensionFound();
          $this->databaseObjects[$mode]->connect($object);
        } else {
  ...
  */
  function escapeString($object, $value, $readOnly = TRUE) {
    ...
    $mode = ($readOnly) ? 'read' : 'write';
    return $this->databaseObjects[$mode]->escapeString($value);
  }
  ...
  */
  function quoteString($object, $value, $readOnly = TRUE) {
    $this->connect($object, $readOnly);
    $mode = ($readOnly) ? 'read' : 'write';
    return $this->databaseObjects[$mode]->quoteString($value);
  }
  ...
  */
  private function _executeQuery($object, $sql, $max = NULL, $offset = NULL, $readOnly = TRUE) {
    ...
    if ($this->debugCounter > 0 ||
        $options->getOption('PAPAYA_LOG_RUNTIME_DATABASE', FALSE) ||
        $options->getOption('PAPAYA_LOG_DATABASE_QUERY', 0) > 0 ||
        $options->getOption('PAPAYA_QUERYLOG', 0) > 0) {
      $measureTime = TRUE;
    }

    $mode = ($readOnly) ? 'read' : 'write';
    $this->connect($object, $readOnly);
    if ($readOnly && $options->getOption('PAPAYA_LOG_DATABASE_CLUSTER_VIOLATIONS', FALSE)) {
    ...
    $result = $this->databaseObjects[$mode]->query(
  ...
  */
  private function _logQueryExecution($timeStart, $query) {
    ...
    if ($this->debugCounter > 0) {
      ...
      switch ($options->getOption('PAPAYA_LOG_DATABASE_QUERY', 0)) {
      case 2 : // all
      ...
      case 1 : //slow queries
        $dispatchLogMessage =
          $options->getOption('PAPAYA_LOG_DATABASE_QUERY_SLOW', 0) < $deltaMilliseconds;
        break;
      }
      switch ($options->getOption('PAPAYA_QUERYLOG', 0)) {
      case 2 : // all
        $populateQueryLog = TRUE;
        break;
      case 1 : //slow queries
        $populateQueryLog =
          $options->getOption('PAPAYA_QUERYLOG_SLOW', 0) < $deltaMilliseconds;
        break;
      }
    }
    if ($dispatchLogMessage || $populateQueryLog) {
      ...
      $explain = NULL;
      $counter = NULL;
      if ($options->getOption('PAPAYA_LOG_DATABASE_QUERY_DETAILS', FALSE)) {
        $dispatchLogMessageDetails = TRUE;
      }
      if ($options->getOption('PAPAYA_QUERYLOG_DETAILS', FALSE)) {
      ...
      if ($dispatchLogMessageDetails || $populateQueryLogDetails) {
        if (is_object($query['result'])) {
          ...
          if (preg_match('(^\s*SELECT)i', $query['sql'])) {
            $explain = $query['result']->getExplain();
          }
        }
      ...
      if ($dispatchLogMessage) {
        ...
        $logMessage->context()->append(
          new PapayaMessageContextVariable(array('sql' => $query['sql']), 2, 99999)
        );
  ...
  */
  function insertRecord($object, $table, $idField, $values = NULL) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->insertRecord($table, $idField, $values);
  }
  ...
  */
  function lastInsertId($object, $table, $idField) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->lastInsertId($table, $idField);
  }
  ...
  */
  function insertRecords($object, $table, array $values) {
    ...
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->insertRecords($table, $values);
  }
  ...
  */
  function loadRecord($object, $table, array $fields, $filter, $value = NULL, $readOnly = FALSE) {
    $this->connect($object, FALSE);
    $mode = ($readOnly) ? 'read' : 'write';
    return $this->databaseObjects[$mode]->loadRecord(
  ...
  */
  function updateRecord($object, $table, array $values, $filter, $value = NULL) {
    ...
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->updateRecord(
  ...
  */
  function deleteRecord($object, $table, $filter, $value = NULL) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->deleteRecord(
  ...
  */
  function queryTableNames($object) {
    $this->connect($object);
    return $this->databaseObjects['read']->queryTableNames();
  }
  ...
  */
  function queryTableStructure($object, $tableName, $tablePrefix = '') {
    $this->connect($object);
    return $this->databaseObjects['read']->queryTableStructure(
  ...
  */
  function createTable($object, $tableData, $tablePrefix) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->createTable($tableData, $tablePrefix);
  }
  ...
  */
  function addField($object, $table, $fieldData) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->addField($table, $fieldData);
  }
  ...
  */
  function changeField($object, $table, $fieldData) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->changeField($table, $fieldData);
  }
  ...
  */
  function dropField($object, $table, $field) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->dropField($table, $field);
  }
  ...
  */
  function addIndex($object, $table, $index) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->addIndex($table, $index);
  }
  ...
  */
  function changeIndex($object, $table, $index) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->changeIndex($table, $index);
  }
  ...
  */
  function getSQLSource($object, $function) {
    ...
    if ($str = $this->databaseObjects['write']->getSQLSource($function, $params)) {
      return $str;
    }
    return FALSE;
  }
  ...
  */
  function getSQLCondition($object, $filter, $value = NULL, $operator = '=') {
    if (!empty($filter)) {
      ...
      if ($str = $this->databaseObjects['read']->getSQLCondition($filter, $operator)) {
        return $str;
      }
    }
    return FALSE;
  ...
  */
  function dropIndex($object, $table, $name) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->dropIndex($table, $name);
  }
  ...
  */
  function compareFieldStructure($object, $xmlField, $databaseField) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->compareFieldStructure(
  ...
  */
  function compareKeyStructure($object, $xmlKey, $databaseKey) {
    $this->connect($object, FALSE);
    return $this->databaseObjects['write']->compareKeyStructure($xmlKey, $databaseKey);
  }
