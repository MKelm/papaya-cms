...
/**
* load basic media db edit functions
*/
...
class papaya_mediadb_mime extends base_mediadb_edit {
  ...
  */
  function execute() {
    ...
    if ($this->authUser->hasPerm(PapayaAdministrationPermissions::SYSTEM_MIMETYPES_MANAGE)) {
  ...
  */
  function executeMimeTypesHandling() {
    if (isset($this->params['cmd'])) {
      ...
      case 'add_group':
        if ($this->authUser->hasPerm(PapayaAdministrationPermissions::SYSTEM_MIMETYPES_EDIT)) {
      ...
      case 'add_ext':
        if ($this->authUser->hasPerm(PapayaAdministrationPermissions::SYSTEM_MIMETYPES_EDIT)) {
          ...
          if (isset($this->params['confirm']) && $this->params['confirm']
              ...
              && checkit::isAlphaNum($this->params['extension'])) {
  ...
  */
  function getMimeGroupPanel() {
    $result = '';
    $groups = $this->mimeObj->getMimeGroups($this->lngSelect->currentLanguageId, TRUE);
        ...
        foreach ($extensions as $extension => $mimetype) {
          ...
          if ($this->authUser->hasPerm(PapayaAdministrationPermissions::SYSTEM_MIMETYPES_EDIT)) {
            $result .= sprintf(
              ...
              papaya_strings::escapeHTMLChars($this->images['actions-generic-delete']),
  ...
  */
  function initializeMimeGroupEditDialog($cmd) {
    ...
    if ($cmd == 'edit_group' &&
      ...
      if (isset($groupData[$this->lngSelect->currentLanguageId])) {
        $data = array(
          'group_title' => $groupData[$this->lngSelect->currentLanguageId]['mimegroup_title'],
          'group_icon' => $groupData[$this->lngSelect->currentLanguageId]['mimegroup_icon'],
        );
      } else {
    ...
    $this->dialog->tokenKeySuffix = 'mimegroupedit';
  ...
  */
  function getMimeGroupDeleteDialog() {
    if (isset($this->params['group_id']) && $this->params['group_id'] > 0) {
      ...
      $mimeGroup = $this->mimeObj->getMimeGroup($this->params['group_id']);
      if (isset($mimeGroup[$this->lngSelect->currentLanguageId])) {
        $title = $mimeGroup[$this->lngSelect->currentLanguageId]['mimegroup_title'];
  ...
  */
  function delMimeGroup() {
    if (isset($this->params['group_id']) && $this->params['group_id'] > 0) {
      ...
      $condition = array('mimegroup_id' => $this->params['group_id']);
      if ($this->databaseDeleteRecord($this->mimeObj->tableMimeGroupsTrans, $condition)) {
        $this->addMsg(
          MSG_INFO,
          sprintf(
        ...
        if ($this->databaseDeleteRecord($this->mimeObj->tableMimeGroups, $condition)) {
          $this->addMsg(
            MSG_INFO,
            sprintf(
  ...
  */
  function initializeMimeTypeEditDialog($cmd) {
    ...
    $mimeGroups = $this->mimeObj->getMimeGroups($this->lngSelect->currentLanguageId);
    ...
    $this->dialog = new base_dialog($this, $this->paramName, $fields, $data, $hidden);
    $this->dialog->tokenKeySuffix = 'mimetypedit';
    $this->dialog->loadParams();
    $this->dialog->dialogTitle = $title;
    $this->dialog->buttonTitle = 'Save';
    $this->dialog->inputFieldSize = 'large';
  ...
  */
  function formatByteValue($bytes) {
    $kBytes = $bytes / 1024;
    if ($bytes >= 1024 && $kBytes == (int)$kBytes) {
      if ($kBytes >= 1024) {
        $mBytes = $kBytes / 1024;
  ...
  */
  function decodeBytesInput($input) {
    if (preg_match('~^(\d+)\s*(bytes|byte|b|bit|kb|kbit|mb|mbit)?(?:/s)?$~i', $input, $matches)) {
      $value = (int)strtolower($matches[1]);
      if (isset($matches[2])) {
      ...
      case 'bit' :
        ...
        break;
      case 'kbit' :
        $value = ceil($value / 8 * 1024);
        break;
      case 'mbit' :
        $value = ceil($value / 8 * 1024 * 1024);
        break;
      case 'kb' :
        $value = $value * 1024;
        break;
  ...
  */
  function addMimeType() {
    ...
    if (is_array($mimeType) && count($mimeType) > 0) {
      ...
      if (isset($mimeGroup[$this->lngSelect->currentLanguageId])) {
        $mimeGroupTitle = $mimeGroup[$this->lngSelect->currentLanguageId]['mimegroup_title'];
      } elseif (count($mimeGroup) > 0) {
    ...
    if (isset($this->params['mimetype_id']) && $this->params['mimetype_id'] > 0) {
      ...
      if (is_array($mimeType) && count($mimeType) > 0) {
        if (count($extensions) == 0 ||
          ...
          if ($this->databaseDeleteRecord($this->mimeObj->tableMimeTypes, $condition)) {
            $this->addMsg(
              MSG_INFO,
              sprintf(
  ...
  */
  function initializeMimeTypeExtensionDialog() {
    ...
    $this->dialog = new base_dialog($this, $this->paramName, $fields, $data, $hidden);
    $this->dialog->tokenKeySuffix = 'mimeextedit';
    $this->dialog->loadParams();
    $this->dialog->dialogTitle = $title;
  ...
  */
  function delMimeTypeExtension($mimeTypeId, $extension) {
    ...
    $this->databaseDeleteRecord($this->mimeObj->tableMimeTypesExtensions, $condition);
    if ($deleted) {
      $this->addMsg(
        MSG_INFO,
        sprintf(
