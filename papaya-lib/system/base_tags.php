...
class base_tags extends base_db {
  ...
  */
  function addCategory($parentCategoryId, $permissionMode, $userType, $userId,
                       $translations) {
  ...
  *
  * @param mixed $categoryId array of category ids or single category id
  * @param integer $lngId language id
  ...
  */
   ...
   * @return array $result category data (cat_id => array(tag_id => array(tag_data)))
   */
  function getTagTitlesByCategory($categoryId, $lngId, $limit = 20, $offset = 0, $char = NULL) {
    $result = array();
    $categoryCondition = str_replace(
        '%', '%%', $this->databaseGetSQLCondition('t.category_id', $categoryId)
    );
    ...
    $sql = "SELECT DISTINCT(tt.tag_title) as title
              FROM %s t
              LEFT OUTER JOIN %s tt ON (t.tag_id = tt.tag_id $lngCondition)
             WHERE $categoryCondition
                   $charCondition
  ...
  function getTagTitlesByParentCategory(
    ...
    $sql = "SELECT DISTINCT(tt.tag_title) as title
              FROM (%s t, %s c)
              LEFT JOIN %s tt ON (t.tag_id = tt.tag_id $lngCondition)
             WHERE $categoryCondition
                   $charCondition
               AND t.category_id = c.category_id
          ORDER BY tag_title ASC";
    $params = array($this->tableTag, $this->tableTagCategory, $this->tableTagTrans);
    if ($res = $this->databaseQueryFmt($sql, $params, $limit, $offset)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        $result[] = $row['title'];
      }
   ...
   * @return array
   */
  public function getCharsByCategory($categoryId, $lngId) {
    $result = array();
    $categoryCondition = str_replace(
        '%', '%%', $this->databaseGetSQLCondition('t.category_id', $categoryId)
    );
    ...
    $sql = "SELECT DISTINCT (tt.tag_char)
              FROM %s AS tt
         LEFT JOIN %s AS t
    ...
    $params = array($this->tableTagTrans, $this->tableTag);
    if ($res = $this->databaseQueryFmt($sql, $params)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        $result[] = $row['tag_char'];
      }
  ...
  public function getCharsByParentCategory($categoryId, $lngId) {
    ...
    $params = array($this->tableTagTrans, $this->tableTag, $this->tableTagCategory);
    if ($res = $this->databaseQueryFmt($sql, $params)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        $result[] = $row['tag_char'];
      }
  ...
  *
  * @param string $type tag type
  * @param array $linkId link id or array of link ids
  ...
  *
  * @param object $authUser instance of base_auth
