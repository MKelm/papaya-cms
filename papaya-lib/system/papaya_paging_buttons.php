...
* @version $Id: papaya_paging_buttons.php 32875 2009-11-04 14:53:32Z weinert $
*/
...
class papaya_paging_buttons {
  ...
  *
  * @param object base_object &$aOwner
  ...
  * @return string
  */
  function getPagingButtons(&$aOwner, $baseParams, $offset, $step, $max,
      $groupCount = 9, $paramName = 'offset', $buttonAlign = NULL) {
      ...
      if ($max > $step) {
        ...
        if ($currentPage > 0) {
          ...
          $result .= sprintf(
            ...
            papaya_strings::escapeHTMLChars($aOwner->images['actions-go-previous']),
            papaya_strings::escapeHTMLChars($aOwner->getLink($params))
          );
        } else {
          $result .= sprintf(
            '<button glyph="%s" />'.LF,
            papaya_strings::escapeHTMLChars($aOwner->images['status-go-previous-disabled'])
          );
        }
        ...
        if ($currentPage < $pageCount - 1) {
          ...
          $result .= sprintf(
            ...
            papaya_strings::escapeHTMLChars($aOwner->images['actions-go-next']),
            papaya_strings::escapeHTMLChars($aOwner->getLink($params))
          );
        } else {
          $result .= sprintf(
            '<button glyph="%s" />'.LF,
            papaya_strings::escapeHTMLChars($aOwner->images['status-go-next-disabled'])
          );
        }
        $result .= '</'.papaya_strings::escapeHTMLChars($positionTag).'>';
        return $result;
  ...
  * @return string
  */
  function getButtons(&$aOwner, $baseParams, $pageValues, $currentValue,
    $paramName = 'limit', $buttonAlign = NULL) {
    if (is_object($aOwner) && is_subclass_of($aOwner, 'base_object')) {
