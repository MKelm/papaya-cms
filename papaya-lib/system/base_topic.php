...
class base_topic extends base_db {
  ...
  * @return boolean
  */
  function loadOutput($topicId, $lngId) {
  ...
  */
  function getViewId() {
    if (isset($this->topic) && $this->topic['TRANSLATION']) {
  ...
  */
  function getBoxParent() {
    if (!empty($this->topic['prev_path']) &&
        preg_match_all('/\d+/', (string)$this->topic['prev_path'], $matches, PREG_PATTERN_ORDER)) {
      $previousIds = $matches[0];
    } else {
      $previousIds = array();
    }
  ...
  */
  function checkURLFileName($currentFileName, $outputMode) {
    if (!empty($currentFileName)) {
      ...
      if ($url) {
        $allowFixation = $this->papaya()->options->getOption('PAPAYA_URL_FIXATION', FALSE);
        if ($allowFixation) {
  ...
  public function getPageLanguage() {
    ...
    } elseif (isset($this->_language)) {
      return $this->_language = $this->papaya()->languages->getLanguage(
