...
class papaya_alias_tree extends base_db {
      ...
      foreach ($this->aliases as $id=>$val) {
        if (isset($val) && is_array($val)) {
          ...
          $result .= sprintf(
            ...
            papaya_strings::escapeHTMLChars($this->images['items-alias']),
            $selected
          );
          if ($val['url_redirectmode'] == 3) {
            ...
            if (isset($val['module_guid']) && !empty($val['module_title'])) {
              $result .= sprintf(
                '<listitem image="%s" title="%s" indent="1" span="2" />'.LF,
                papaya_strings::escapeHTMLChars($this->images['items-plugin']),
  ...
  * @return string $result
  */
  function getSubElements($id, $caller, &$counter) {
    $result = "";
  ...
  function initializeAliasDialog() {
    if (!(isset($this->dialogAlias) && is_object($this->dialogAlias))) {
      ...
      if (!isset($data['url_redirectmode']) || $data['url_redirectmode'] < 2) {
        ...
        $this->aliasPlugin = &base_pluginloader::getPluginInstance($moduleGuid, $this);
        if (isset($this->aliasPlugin) &&
            isset($this->aliasPlugin->editFields) && is_array($this->aliasPlugin->editFields)) {
          $this->aliasPlugin->msgs = $this->msgs;
          $this->aliasPlugin->paramName = $this->paramName;
          $this->aliasPlugin->setData(
            empty($data['module_params']) ? '' : (string)$data['module_params']
          );
          if (isset($this->aliasPlugin->data) && is_array($this->aliasPlugin->data)) {
            $data = array_merge($data, $this->aliasPlugin->data);
          }
      ...
      $this->dialogAlias->msgs = &$this->msgs;
