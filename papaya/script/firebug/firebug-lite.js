  ...
  d: {
    ...
    console:{
      ...
      cmd:{
        ...
        group:function(_value){
          with(firebug){
            var args = d.console.formatArgs.apply(window,arguments);
            d.console.addLine().attribute.addClass("Group").update(args);
          }
        },
        groupEnd:function(){
          with(firebug){
        ...
        try {
          ...
          if(_value==null||vtype=="number"||vtype=="string"||vtype=="boolean"||(vtype=="function"&&_value.nodeName!="OBJECT")||vtype=="regexp"||vtype=="date"){
            if(_value==null){
              ...
              result.push("<span>\""+( !_inObject&&!_inArray?_value : _value.substring(0,35)+(_value.length>35?" ...":"") ).replace(/\n/g,"\\n").replace(/\s/g,"&nbsp;").replace(/>/g,"&#62;").replace(/</g,"&#60;").replace(/\\n/g,"<br/>")+"\"</span>");
            }
          }
          // element
          else if(isElement){
